...
wecoffeeStyle.innerHTML = `
    ...
    /* ===== 모바일 전체화면 버튼 ===== */
    #timeline-fullscreen-btn { display: none; }
    @media (max-width: 768px) {
        ...
        #timeline-fullscreen-btn:active { background: #f2f4f6; }
        #timeline-fullscreen-btn svg { width: 16px; height: 16px; stroke: var(--text-secondary); }

        #timeline-landscape-overlay {
            display: none;
            position: fixed;
            top: 0; left: 0;
            width: 100vh;
        ...
        #timeline-landscape-overlay.active { display: block; }
        #timeline-landscape-close {
            position: sticky; top: 0; left: 0; z-index: 10;
            display: flex; align-items: center; justify-content: space-between;
            margin-bottom: 12px; background: #fff;
            padding: 4px 0 8px; border-bottom: 1px solid var(--border-strong);
        }
        #timeline-landscape-close span { font-size: 15px; font-weight: 800; color: var(--text-display); }
        #timeline-landscape-close button {
            padding: 6px 14px; border-radius: 8px;
            border: 1px solid var(--border-strong); background: #f9fafb;
            font-size: 13px; font-weight: 700; cursor: pointer; color: var(--text-display);
        }
        #timeline-landscape-inner .timeline-section {
            padding: 12px 8px; margin-bottom: 20px !important;
            border-radius: 10px; border: 1px solid var(--border-strong);
            background: #fff; box-shadow: none;
        }
        #timeline-landscape-inner .timeline-grid { min-width: 900px; }
    }
`;
...
window.toggleDashView = function(view) { 
    ...
    if(view === 'month') { if($("dashMonthNav")) $("dashMonthNav").style.display = 'flex'; } 
    else { if($("dashMonthNav")) $("dashMonthNav").style.display = 'none'; currentDashMonthOffset = 0; } 
...
// ===== 전체화면 오버레이 열기/닫기 =====
window.openTimelineLandscape = function() {
    let overlay = document.getElementById('timeline-landscape-overlay');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'timeline-landscape-overlay';
        overlay.innerHTML = `
            <div id="timeline-landscape-close">
                <span>실시간 센터 현황</span>
                <button onclick="window.closeTimelineLandscape()">✕ 닫기</button>
            </div>
            <div id="timeline-landscape-inner"></div>
        `;
        document.body.appendChild(overlay);
    }
    // 현재 타임라인 내용 복사
    let source = document.getElementById('timeline-area');
    let inner = document.getElementById('timeline-landscape-inner');
    if (source && inner) {
        inner.innerHTML = source.innerHTML;
        // 복사본에서 전체화면 버튼 제거
        let btn = inner.querySelector('#timeline-fullscreen-btn');
        if (btn) btn.remove();
    }
...
window.closeTimelineLandscape = function() {
    let overlay = document.getElementById('timeline-landscape-overlay');
    ...
    let finalHtml = `
        <div class="timeline-section">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 8px;">
                ...
                <button id="timeline-fullscreen-btn" onclick="window.openTimelineLandscape()" title="전체화면으로 보기">
...
window.renderDashboard = async function() {
    ...
    try {
        gRes.forEach(r => {
            ...
            if (calEvts[r.res_date]) { let st = String(r.res_time||"").split('~')[0].trim(); let spc = String(r.space_equip||"").split(' ')[0]; calEvts[r.res_date].push({ time: r.res_time, start: st, text: `[${spc}] ${r.name}`, type: 'res', tooltip: `${r.res_time} | ${r.space_equip} | ${r.name}` }); }
        });
        gBlk.forEach(b => {
            if ((currentGlobalCenter !== '전체' && b.center !== currentGlobalCenter) || (spaceFilter !== '전체' && !String(b.space_equip||'').includes(spaceFilter))) return;
            if (calEvts[b.block_date]) { let timeStr = `${b.start_time}~${b.end_time}`; calEvts[b.block_date].push({ time: timeStr, start: b.start_time, text: `[${b.category}] ${b.reason}`, type: 'blk', tooltip: `${timeStr} | ${b.space_equip||'전체'} | ${b.reason}` }); }
        });
        gTrn.forEach(t => {
            ...
            if(cInfo.length >= 5) {
                ...
                if (calEvts[tDate]) { let st = String(cInfo[2]||"").split('~')[0].trim(); calEvts[tDate].push({ time: cInfo[2], start: st, text: `[수강] ${t.name}`, type: 'trn', tooltip: `${cInfo[2]} | ${tSpc} | ${t.name} (${cInfo[1]})` }); }
            }
        });
    } catch(e) { console.error(e); }
    try {
...
window.renderAppDailyBanner = function(filteredApps) {
    ...
    if(todayEvts.length === 0) { html = `<div class="inout-card"><div style="font-weight:800; margin-bottom:8px; color:var(--text-display); border-bottom:1px solid var(--border-strong); padding-bottom:8px;">오늘의 상담 일정</div><div style="font-size:13px; color:var(--text-secondary); padding:8px 0;">오늘 확정된 상담 일정이 없습니다.</div></div>`; } 
        ...
        todayEvts.forEach(evt => { const tm = String(evt.call_time||'').match(/(오전|오후)\s+(\d+)[시:]\s*(\d+)?/); let timeStr = tm ? `${tm[1]} ${tm[2]}:${tm[3] ? String(parseInt(tm[3], 10)).padStart(2,'0') : '00'}` : evt.call_time; html += `<div style="display:flex; align-items:center; flex-wrap:wrap; gap:6px; margin-bottom:4px; width:100%;"><div class="today-time-wrap" style="color:var(--primary); background:var(--primary-light); padding:4px 8px; border-radius:4px; font-size:12px; font-weight:700; white-space:nowrap; flex-shrink:0;">${timeStr}</div> <div class="today-time-wrap" style="font-weight:800; flex-shrink:0;">[${evt.desired_batch||'-'}] ${window.escapeHtml(evt.name)}</div> <div style="font-weight:500; color:var(--text-secondary); flex-shrink:0;">(${window.escapeHtml(evt.phone)}) | 담당: ${window.escapeHtml(evt.counselor_name||'미정')}</div></div>`; });
        html += `</div></div>`; 
    }
...
window.openCrmModal = function(id, isReadOnly = false) {
    isCrmReadOnly = isReadOnly;
    if($("crmAppId")) $("crmAppId").value = id;
    window.renderCrmInner(id, isReadOnly);
...
window.fetchMembers = async function() { 
    ...
    let bHtml = `<option value="all">기수 전체</option>` + Array.from(bSet).sort((a,b) => parseInt(String(a).replace(/[^0-9]/g, '') || 0) - parseInt(String(b).replace(/[^0-9]/g, '') || 0)).map(b=>`<option value="${b}">${b}</option>`).join(""); 
...
window.searchMembers = function() { 
    ...
    let filtered = globalMembers.filter(m => { 
        ...
        else if (statusFilter === '활동 중 (전체)') matchStatus = ['활동 중', '연장 활동 중', '단일권 이용'].includes(statusText); 
        else matchStatus = statusText === statusFilter; 
        return matchQuery && matchStatus && matchBatch; 
    }); 
    filtered.sort((a, b) => { let batchA = a.batch || ''; let batchB = b.batch || ''; if (batchA !== batchB) { return parseInt(String(batchB).replace(/[^0-9]/g, '') || 0) - parseInt(String(batchA).replace(/[^0-9]/g, '') || 0); } return (a.name || '').localeCompare(b.name || ''); });
    currentFilteredMembers = filtered; currentMemberPage = 1;
    let filterWrap = document.querySelector('#page-members .filter-wrap');
    if(filterWrap && !document.getElementById('memberPerPage')) { let selectHtml = `<select id="memberPerPage" class="filter-sel" style="margin-left:8px; width:auto;" onchange="window.changeMemberPerPage(this.value)"><option value="10">10명씩 보기</option><option value="50" selected>50명씩 보기</option><option value="100">100명씩 보기</option><option value="all">전체 보기</option></select>`; filterWrap.insertAdjacentHTML('beforeend', selectHtml); }
...
window.renderMemberTablePage = function() {
  ...
  if(data.length === 0) { tbody.innerHTML = `<tr><td colspan="8" class="empty-state">내역이 없습니다.</td></tr>`; updatePaginationUI(0); return; } 
  let memTable = tbody.closest('table');
  if (memTable) { let theadTr = memTable.querySelector('thead tr'); if (theadTr) { let firstTh = theadTr.querySelector('th'); if (firstTh && !firstTh.querySelector('input[type="checkbox"]') && firstTh.innerText.includes('등록일')) { let chkTh = document.createElement('th'); chkTh.style.width = '48px'; chkTh.style.textAlign = 'center'; chkTh.innerHTML = '<input type="checkbox" onchange="window.toggleAll(this, \'chk-mem\')">'; theadTr.insertBefore(chkTh, firstTh); } } }
  let startIndex = (currentMemberPage - 1) * memberItemsPerPage; let endIndex = startIndex + memberItemsPerPage; let pageData = data.slice(startIndex, endIndex);
  const today = new Date(); today.setHours(0,0,0,0); let now = new Date(); let monthPrefix = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
  let getCancelCount = (phone) => { let count = 0; gRes.forEach(r => { if(r.phone === phone && r.status === '당일 취소' && String(r.res_date || r.created_at).startsWith(monthPrefix)) count++; }); gTrn.forEach(t => { if(t.phone === phone && t.status === '당일 취소') { let dStr = String(t.content||'').split(' || ')[0] || String(t.created_at); if(dStr.startsWith(monthPrefix)) count++; } }); return count; };
...
function updatePaginationUI(totalItems) {
    ...
    if(!paginationWrap) { let tableWrap = document.querySelector('#page-members .table-wrap'); if(tableWrap) { paginationWrap = document.createElement('div'); paginationWrap.id = 'memberPaginationWrap'; paginationWrap.style.cssText = 'display:flex; justify-content:center; align-items:center; gap:8px; padding:20px 0;'; tableWrap.parentNode.insertBefore(paginationWrap, tableWrap.nextSibling); } }
    if(!paginationWrap) return;
    if(totalItems === 0 || memberItemsPerPage > totalItems) { paginationWrap.innerHTML = ''; return; }
    let totalPages = Math.ceil(totalItems / memberItemsPerPage); let html = '';
    let startPage = Math.max(1, currentMemberPage - 2); let endPage = Math.min(totalPages, startPage + 4); if(endPage - startPage < 4) startPage = Math.max(1, endPage - 4);
    if(currentMemberPage > 1) { html += `<button class="pagination-btn" onclick="window.changeMemberPage(${currentMemberPage - 1})">이전</button>`; }
    for(let i = startPage; i <= endPage; i++) { let activeClass = i === currentMemberPage ? 'active' : ''; html += `<button class="pagination-btn ${activeClass}" onclick="window.changeMemberPage(${i})">${i}</button>`; }
    if(currentMemberPage < totalPages) { html += `<button class="pagination-btn" onclick="window.changeMemberPage(${currentMemberPage + 1})">다음</button>`; }
    paginationWrap.innerHTML = html;
}
...
window.handleMemberOption = function(id, batch, name, phone, currentEndDate, selectEl) {
  ...
  window.openCustomConfirm(`[${batch || '미정'}] ${name} 님`, statText, confirmMsg, async () => {
      if(opt === 'release') { const m = globalMembers.find(x => String(x.id) === String(id)); let newStat = m.status === '패널티 정지' ? '활동 중' : '패널티 정지'; m.status = newStat; window.searchMembers(); await supabaseClient.from('members').update({ status: newStat }).eq('id', id); showToast(`상태가 [${newStat}] 상태로 변경되었습니다.`); return; }
...
window.saveAdminNote = async function() {
    if(!$("crmAppId")) return; const id = $("crmAppId").value; const app = globalApps.find(a => String(a.id) === String(id)); if(!app) { showToast("신청 정보를 찾을 수 없습니다."); return; }
    const title = $("crmNoteTitle") ? $("crmNoteTitle").value.trim() : ""; const content = $("crmNoteInput") ? $("crmNoteInput").value.trim() : "";
    if(!title && !content) return showToast("내용을 입력해주세요.");
    const newNote = `${title}:::${content}`; let updatedMemo = app.admin_memo ? app.admin_memo + '|||' + newNote : newNote;
    const originalMemo = app.admin_memo; app.admin_memo = updatedMemo; window.renderCrmInner(id, isCrmReadOnly);
    const { error } = await supabaseClient.from('applications').update({ admin_memo: updatedMemo }).eq('id', id);
    if(error) { app.admin_memo = originalMemo; window.renderCrmInner(id, isCrmReadOnly); showToast("기록 추가에 실패했습니다."); console.error(error); } else { showToast("상담 기록이 추가되었습니다."); }
}

window.openCrmModalFromPhone = async function(phone) {
    if(!phone || phone === '-') return showToast("연락처 정보가 없어 설문 내역을 찾을 수 없습니다.");
    const normalizePhone = p => String(p).replace(/\D/g, ''); const targetPhone = normalizePhone(phone);
    let app = globalApps.find(a => normalizePhone(a.phone) === targetPhone);
    if (app) { window.openCrmModal(app.id, true); } 
    else { showToast("내역을 불러오는 중입니다..."); const { data, error } = await supabaseClient.from('applications').select('*'); if (!error && data) { let matched = data.find(a => normalizePhone(a.phone) === targetPhone); if (matched) { if(!globalApps.find(a => String(a.id) === String(matched.id))) globalApps.push(matched); window.openCrmModal(matched.id, true); return; } } showToast("해당 멤버의 가입 신청/설문 내역을 찾을 수 없습니다."); }
}
...
window.sendToGoogleSheet = async function() {
    ...
    const GAS_URL = 'https://script.google.com/macros/s/AKfycbynlyczuJ5VWzfG5IFOstLzkRybv4Yvjgo9bxDHoUQlK84gAehaTuCNommlmrXuFsJK/exec'; 
    const btn = document.getElementById('btn-send-sheet'); if(btn) { btn.innerText = '전송 중...'; btn.disabled = true; }
    try {
        let uniqueMembers = [...new Set(window.currentSummaryData.map(d => d['성함']))].filter(name => name !== "이름없음" && name !== "");
        let invoiceData = uniqueMembers.map(name => { let info = window.currentMemberInfoMap && window.currentMemberInfoMap[name] ? window.currentMemberInfoMap[name] : { phone: '', batch: '' }; return { "등록 일시": "", "발주 구분": "", "수령 센터": "", "생두사": "", "상품명": `[${name}] 님 최종 청구 금액`, "주문 수량": "", "결제 금액": `CALC_TOTAL:${name}`, "기수": info.batch, "성함": name, "연락처": info.phone }; });
        let payload = [...window.currentSummaryData, { "등록 일시": "", "발주 구분": "", "수령 센터": "", "생두사": "", "상품명": "--- ▼ 멤버별 총 결제 금액 명세서 ▼ ---", "주문 수량": "", "결제 금액": "", "기수": "", "성함": "", "연락처": "" }, ...invoiceData];
        await fetch(GAS_URL, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        showToast("구글 시트에 명세서와 함께 전송되었습니다.");
...
window.openBlockModal = function(dateStr, timeStr) {
    ...
    if($("blockId")) $("blockId").value = ''; if($("blkId")) $("blkId").value = ''; 
    ...
    if($("blkReason")) $("blkReason").value = ''; if($("blkCapacity")) $("blkCapacity").value = '';
    if($("blockModalTitle")) $("blockModalTitle").innerText = "신규 스케줄 등록";
    let modal = document.getElementById("blockModal") || document; let rSel = Array.from(modal.querySelectorAll('select')).find(s => s.options && Array.from(s.options).some(opt => opt.value === '매일')); if(rSel) { rSel.value = '반복 없음'; let wrap = rSel.closest('div[style*="flex"]') || rSel.parentElement; if(wrap) wrap.style.display = ''; }
    let rInp = Array.from(modal.querySelectorAll('input[type="number"], input[type="text"]')).find(i => i.placeholder && i.placeholder.includes('4')); if(!rInp) rInp = Array.from(modal.querySelectorAll('input')).find(i => i.previousElementSibling && i.previousElementSibling.textContent.includes('횟수')); if(rInp) { rInp.value = ''; let wrap = rInp.closest('div[style*="flex"]') || rInp.parentElement; if(wrap) wrap.style.display = ''; }
};

window.editBlock = function(id) {
    let b = gBlk.find(x => String(x.id) === String(id)); if(!b) return;
    if($("blockModal")) $("blockModal").classList.add('show');
    if($("blockId")) $("blockId").value = b.id; if($("blkId")) $("blkId").value = b.id; 
    if($("blkDate")) $("blkDate").value = b.block_date; if($("blkStart")) $("blkStart").value = b.start_time; if($("blkEnd")) $("blkEnd").value = b.end_time;
    if($("blkCategory")) $("blkCategory").value = b.category; if($("blkCenter")) $("blkCenter").value = b.center || '마포 센터';
    if(window.updateSpaceOptions) window.updateSpaceOptions();
    if($("blkSpace")) { $("blkSpace").value = b.space_equip || ''; $("blkSpace").dataset.selectedValues = b.space_equip || ''; }
    if($("blkReason")) $("blkReason").value = b.reason; if($("blkCapacity")) $("blkCapacity").value = b.capacity === null ? '' : b.capacity;
    if($("blockModalTitle")) $("blockModalTitle").innerText = "스케줄 수정";
    let modal = document.getElementById("blockModal") || document; let rSel = Array.from(modal.querySelectorAll('select')).find(s => s.options && Array.from(s.options).some(opt => opt.value === '매일')); if(rSel) { let wrap = rSel.closest('div[style*="flex"]') || rSel.parentElement; if(wrap) wrap.style.display = 'none'; }
    let rInp = Array.from(modal.querySelectorAll('input[type="number"], input[type="text"]')).find(i => i.placeholder && i.placeholder.includes('4')); if(!rInp) rInp = Array.from(modal.querySelectorAll('input')).find(i => i.previousElementSibling && i.previousElementSibling.textContent.includes('횟수')); if(rInp) { let wrap = rInp.closest('div[style*="flex"]') || rInp.parentElement; if(wrap) wrap.style.display = 'none'; }
};
...
window.saveBlockData = async function() {
    ...
    let capVal = $("blkCapacity") ? $("blkCapacity").value.trim() : ""; let spaceVal = $("blkSpace") ? $("blkSpace").value.trim() : "전체";
    let baseDateStr = $("blkDate") ? $("blkDate").value : ""; let startTime = $("blkStart") ? $("blkStart").value : ""; let endTime = $("blkEnd") ? $("blkEnd").value : "";
    let category = $("blkCategory") ? $("blkCategory").value : "수업"; let center = $("blkCenter") ? $("blkCenter").value : "마포 센터"; let reason = $("blkReason") ? $("blkReason").value : ""; let capacity = capVal === "" ? null : parseInt(capVal);
    ...
    if(!id || id === "") { let modal = document.getElementById("blockModal") || document; let rSel = Array.from(modal.querySelectorAll('select')).find(s => s.options && Array.from(s.options).some(opt => opt.value === '매일')); if(rSel) repeatType = rSel.value; let rInp = Array.from(modal.querySelectorAll('input[type="number"], input[type="text"]')).find(i => i.placeholder && i.placeholder.includes('4')); if(!rInp) rInp = Array.from(modal.querySelectorAll('input')).find(i => i.previousElementSibling && i.previousElementSibling.textContent.includes('횟수')); if(rInp && rInp.value) { let parsed = parseInt(rInp.value.replace(/[^0-9]/g, '')); if(!isNaN(parsed) && parsed > 0) repeatCount = parsed; } }
    let payloads = []; let baseDate = new Date(baseDateStr);
    for (let i = 0; i < repeatCount; i++) { let targetDate = new Date(baseDate); if (repeatType.includes("매일")) { targetDate.setDate(targetDate.getDate() + i); } else if (repeatType.includes("매주") || repeatType.includes("요일반복")) { targetDate.setDate(targetDate.getDate() + (i * 7)); } let yyyy = targetDate.getFullYear(); let mm = String(targetDate.getMonth() + 1).padStart(2, '0'); let dd = String(targetDate.getDate()).padStart(2, '0'); let newDateStr = `${yyyy}-${mm}-${dd}`; payloads.push({ block_date: newDateStr, start_time: startTime, end_time: endTime, category: category, center: center, space_equip: spaceVal || "전체", reason: reason, capacity: capacity }); }
    let error; if(id && id !== "") { const res = await supabaseClient.from('blocks').update(payloads[0]).eq('id', id); error = res.error; } else { const res = await supabaseClient.from('blocks').insert(payloads); error = res.error; }
    window.isSavingBlock = false;
    if(error) { showToast("저장 실패"); console.error(error); } else { showToast(payloads.length > 1 ? `${payloads.length}개의 스케줄이 반복 등록되었습니다.` : "저장되었습니다."); window.closeBlockModal(); window.fetchCenterData(); }
};
...
window.saveNoticeData = async function() { 
  ...
  if(error) { showToast("저장 실패: " + (error.message || error.details || "알 수 없는 오류")); console.error("Notice Save Error:", error); } else { showToast("저장되었습니다."); window.closeNoticeModal(); window.fetchCenterData(); } 
}
