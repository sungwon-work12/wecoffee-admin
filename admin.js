...
function wcRefUpdateCvaPreview() {
  const valEl = $("refCvaVal"); if (!valEl) return;
  const qual = {};
  CUPPING_QUAL_KEYS.forEach(function (k) { const el = $(k); const v = el ? String(el.value || "").trim() : ""; qual[k] = (v === "" ? null : Number(v)); });
  ...
  valEl.textContent = (cva == null ? "—" : cva.toFixed(1));
  valEl.style.color = (cva == null ? "#b0b8c1" : "#191f28");
}
