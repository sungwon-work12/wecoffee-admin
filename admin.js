...
// ▼▼▼ 파트1 끝 — 다음은 toggleDashView, renderCenterData, renderTimeline 등 (파트2) ▼▼▼'// ▼▼▼ 파트2 시작 (toggleDashView 부터) ▼▼▼
...
function generateOrderRows(fOrd, chkClass, dateKey) {
  return fOrd.map(o => {
    ...
    let paySelect = `<select class="status-select ${payClass}" onchange="window.handlePaymentStatus('${o.id}',this.value,this)" style="font-size:12px;padding:6px 20px 6px 8px;width:100%;text-align-last:center;" ${payDisabled}><option value="주문 접수" ${payStatus==='주문 접수'?'selected':''}>주문 접수</option><option value="입금 대기" ${payStatus==='입금 대기'?'selected':''}>입금 대기</option><option value="입금 확인 중" ${payStatus==='입금 확인 중'?'selected':''}>입금 확인 중</option><option value="입금 확인" ${payStatus==='입금 확인'?'selected':''}>입금 확인</option><option value="주문 취소" ${payStatus==='주문 취소'?'selected':''}>주문 취소</option><option value="품절" ${payStatus==='품절'?'selected':''}>품절</option></select>`;
    let delSelect = `<select class="status-select ${delClass}" onchange="window.handleDeliveryStatus('${o.id}',this.value,this)" style="font-size:12px;padding:6px 20px 6px 8px;width:100%;text-align-last:center;" ${delEnabled?'':'disabled'}><option value="" ${!isArrived?'selected':''}>—</option><option value="센터 도착" ${isArrived?'selected':''}>센터 도착</option></select>`;
