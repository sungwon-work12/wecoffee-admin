...
// 🔥 1. 복사하기 팝업 동기화 완벽 해결 (이벤트 재생성 방식 도입)
...
window.renderAppDashboard = async function() {
    ...
    scheduledApps.forEach(app => { 
        ...
        if (m) { 
            ...
            if (calEvts[ds]) { 
                let timeStr = app.call_time.includes('오전') ? '오전 ' + app.call_time.split('오전')[1].trim() : (app.call_time.includes('오후') ? '오후 ' + app.call_time.split('오후')[1].trim() : app.call_time);
...
// 🔥 3. 오늘의 상담 일정 - 모바일 세로형 스크롤 삭제 및 PC 가로 그리드 배치 (완벽 최적화)
window.renderAppDailyBanner = function(data) {
  ...
  if(todayEvts.length === 0) { 
      ...
      html = `<div style="width:100%; display:grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap:16px;">`; 
      todayEvts.sort((a,b) => String(a.call_time||'').localeCompare(String(b.call_time||''))); 
      todayEvts.forEach(evt => { 
          let timeStr = evt.call_time.includes('오전') ? '오전 ' + evt.call_time.split('오전')[1].trim() : (evt.call_time.includes('오후') ? '오후 ' + evt.call_time.split('오후')[1].trim() : evt.call_time);
...
// 🔥 3. 모바일 CRM "유입 경로" 핵심 파싱 및 토스/현대카드 스타일 UI 적용 (이모지 X)
...
// 🔥 4. 멤버 이관: 당월 15일 이전/이후 기준 자동 날짜 적용
window.updateAppStatus = async function(id, column, value) {
    ...
    if (column === 'join_status' && value === '가입 완료') {
        window.openCustomConfirm("가입 완료 (멤버 전환)", null, `해당 고객을 멤버 리스트로 이관하시겠습니까?<br><span style="font-size:12px; color:var(--text-secondary); font-weight:500; display:block; margin-top:8px; line-height:1.5;">당월 15일 이전을 기준으로 6개월 후 활동 종료일이 자동 세팅됩니다.</span>`, async () => {
