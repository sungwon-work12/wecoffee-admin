...
// ▼▼▼ 커핑 관리 모듈 시작 ▼▼▼
// 삭제 시: 이 블록 전체 제거 + 02-modal-html.html 제거 + 01-migration.sql의 롤백 실행

let gCuppingBeans = {};

// ── saveBlockData 패치: 커핑 카테고리 블록 저장 시 cupping_session 자동 생성 ──
const _origSaveBlockData = window.saveBlockData;
window.saveBlockData = async function() {
  // 저장 전 카테고리 확인
  const category = document.getElementById('blkCategory') ? document.getElementById('blkCategory').value : '';
  const isCupping = category.includes('커핑');
  const editId = document.getElementById('blkId') ? document.getElementById('blkId').value : '';
  ...
  // 커핑이면 세션 자동 생성/연결
  if (isCupping && !editId) {
    // 방금 생성된 블록 찾기 (가장 최근 것)
    const { data: latestBlocks } = await supabaseClient
      .from('blocks')
...
// ── renderCenterData 패치: 커핑 블록에 "커핑 설정" 버튼 추가 ──
const _origRenderCenterData = window.renderCenterData;
window.renderCenterData = function() {
  _origRenderCenterData();

  // 블록 테이블에서 커핑 카테고리인 행에 버튼 추가
  const blkBody = document.getElementById('blkTableBody');
  if (!blkBody) return;

  const rows = blkBody.querySelectorAll('tr');
  rows.forEach(tr => {
    // 카테고리 셀 찾기 (data-label="구분")
    const catCell = tr.querySelector('td[data-label="구분"]');
    if (!catCell) return;
    const catText = catCell.textContent || '';
    if (!catText.includes('커핑')) return;

    // 이미 버튼 있으면 스킵
    if (tr.querySelector('.cupping-setup-btn')) return;

    // 관리 셀에 버튼 추가
    const actionCell = tr.querySelector('td[data-label="관리"]');
    if (!actionCell) return;
    const wrap = actionCell.querySelector('.action-wrap-flex');
    if (!wrap) return;

    // 블록 ID 추출 (수정 버튼의 onclick에서)
    const editBtn = wrap.querySelector('button[onclick*="editBlock"]');
    if (!editBtn) return;
    const match = editBtn.getAttribute('onclick').match(/editBlock\('([^']+)'\)/);
    if (!match) return;
    ...
    btn.onclick = function(e) {
      e.stopPropagation();
      window.openCuppingSetup(blockId);
    };
    wrap.insertBefore(btn, wrap.firstChild);

    // 모바일 프리뷰에도 힌트 추가
    const mPreview = tr.querySelector('.m-prev-desc');
    if (mPreview && !mPreview.querySelector('.cupping-hint')) {
      const hint = document.createElement('span');
      hint.className = 'cupping-hint';
      hint.style.cssText = 'color:var(--primary);font-weight:700;margin-left:8px;font-size:12px;cursor:pointer;';
      hint.textContent = '커핑 설정 →';
      hint.onclick = function(e) {
        e.stopPropagation();
        window.openCuppingSetup(blockId);
      };
      mPreview.appendChild(hint);
    }
  });
};

// ── 커핑 설정 열기 (블록 → 세션 연결 → 라인업 모달) ──
window.openCuppingSetup = async function(blockId) {
  ...
  // 세션 없으면 자동 생성
  if (!session) {
    const blk = gBlk.find(b => String(b.id) === String(blockId));
    ...
    const { data: newSession, error } = await supabaseClient.from('cupping_sessions').insert([{
      block_id: blockId,
      slug,
      title: blk.reason || '커핑 세션',
      type: 'sensory_training',
      center: centerCode,
      scheduled_at: scheduledAt,
      duration_min: duration,
...
window.openCuppingLineup = async function(session) {
  document.getElementById('lineupSessionId').value = session.id;
  document.getElementById('lineupModalTitle').textContent = session.title + ' — 커핑 설정';

  // URL 표시
  const baseUrl = 'https://www.wecoffee.co.kr/cupping/';
  document.getElementById('sessionUrlText').textContent = baseUrl + session.slug;

  // 입력 초기화
  ['beanName','beanOrigin','beanFarm','beanProcess','beanAltitude','beanVariety','beanRoast'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });

  await window.fetchCuppingBeans(session.id);
  document.getElementById('cuppingLineupModal').classList.add('show');
};

window.closeCuppingLineupModal = function() {
  document.getElementById('cuppingLineupModal').classList.remove('show');
};

window.copyCuppingUrl = function() {
  const url = document.getElementById('sessionUrlText').textContent;
  window.copyTxt(url, '커핑 세션 URL이 복사되었습니다.');
};
...
window.renderCuppingBeans = function(sessionId) {
  ...
  const area = document.getElementById('beanListArea');
  const countEl = document.getElementById('beanCount');
  if (countEl) countEl.textContent = beans.length;
  ...
  area.innerHTML = beans.map((b, idx) => {
    ...
    return `<div style="background:#fff;border:1px solid var(--border-strong);border-radius:12px;padding:14px 16px;display:flex;justify-content:space-between;align-items:flex-start;gap:12px;">
      <div style="flex:1;min-width:0;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
          <span style="width:24px;height:24px;border-radius:7px;background:var(--btn-gray);color:var(--mid-gray,#4e5968);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;flex-shrink:0;">${idx + 1}</span>
          <span style="font-size:15px;font-weight:700;color:var(--text-display);">${window.escapeHtml(b.name)}</span>
        </div>
        ${specs.length ? `<div style="font-size:12px;color:var(--text-secondary);margin-left:32px;">${specs.map(s => window.escapeHtml(s)).join(' · ')}</div>` : ''}
        ${b.farm ? `<div style="font-size:12px;color:var(--text-tertiary);margin-left:32px;">농장: ${window.escapeHtml(b.farm)}</div>` : ''}
      </div>
      <div style="display:flex;gap:4px;flex-shrink:0;">
        ${idx > 0 ? `<button class="btn-outline btn-sm" style="padding:4px 8px;" onclick="window.moveCuppingBean('${sessionId}','${b.id}','up')">↑</button>` : ''}
        ${idx < beans.length - 1 ? `<button class="btn-outline btn-sm" style="padding:4px 8px;" onclick="window.moveCuppingBean('${sessionId}','${b.id}','down')">↓</button>` : ''}
...
window.addCuppingBean = async function() {
  const sessionId = document.getElementById('lineupSessionId').value;
  ...
  const { error } = await supabaseClient.from('cupping_beans').insert([{
    ...
    sort_order: beans.length,
    name,
    origin: document.getElementById('beanOrigin').value.trim() || null,
    farm: document.getElementById('beanFarm').value.trim() || null,
    process: document.getElementById('beanProcess').value.trim() || null,
    altitude: document.getElementById('beanAltitude').value.trim() || null,
    variety: document.getElementById('beanVariety').value.trim() || null,
    roast_level: document.getElementById('beanRoast').value.trim() || null
  }]);
  ...
  document.getElementById('beanName').focus();
...
window.deleteCuppingBean = function(sessionId, beanId) {
  window.openCustomConfirm('원두 삭제', null, '이 원두를 라인업에서 삭제하시겠습니까?', async () => {
...
window.moveCuppingBean = async function(sessionId, beanId, direction) {
  const beans = gCuppingBeans[sessionId] || [];
  const idx = beans.findIndex(b => String(b.id) === String(beanId));
  if (idx < 0) return;
  const swapIdx = direction === 'up' ? idx - 1 : idx + 1;
  if (swapIdx < 0 || swapIdx >= beans.length) return;

  const a = beans[idx], b = beans[swapIdx];
  await Promise.all([
...
<!-- ▼ 02-modal-html.html의 beanListArea 닫는 </div> 뒤, 모달 body 닫기 전에 추가 ▼ -->

<div style="border-top:1px solid var(--border-strong);margin-top:20px;padding-top:20px;">
  <!-- 참가자 사전등록 -->
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
    <div style="font-size:14px;font-weight:700;color:var(--text-display);">참가자 (<span id="partCount">0</span>)</div>
    <button class="btn-outline btn-sm" onclick="window.togglePreRegForm()">사전등록</button>
  </div>

  <!-- 사전등록 폼 (토글) -->
  <div id="preRegForm" style="display:none;padding:12px;background:#f9fafb;border-radius:10px;border:1px solid var(--border-strong);margin-bottom:12px;">
    <div style="display:flex;gap:6px;margin-bottom:8px;">
      <input type="text" id="preRegName" class="input-search" style="flex:1;box-sizing:border-box;" placeholder="성함">
      <input type="text" id="preRegPhone" class="input-search" style="flex:1;box-sizing:border-box;" placeholder="연락처">
    </div>
    <div style="display:flex;gap:6px;">
      <select id="preRegType" class="filter-select" style="flex:1;box-sizing:border-box;text-align:left;">
        <option value="member">멤버 (이름으로 검색)</option>
        <option value="guest">게스트</option>
      </select>
      <button class="btn-primary btn-sm" onclick="window.preRegParticipant()">등록</button>
    </div>
  </div>

  <!-- 참가자 목록 -->
  <div id="partListArea" style="display:flex;flex-direction:column;gap:6px;">
    <div class="empty-state" style="padding:20px 0;font-size:13px;">참가자가 없습니다.</div>
  </div>
</div>


<!-- ============================================
     ▼ admin.js 커핑 모듈 하단에 추가할 JS ▼
     ============================================ -->
...
window.renderCuppingParticipants = function(sessionId) {
  ...
  const area = document.getElementById('partListArea');
  ...
  area.innerHTML = parts.map(p => {
    ...
    const name = isMember ? (p.members?.name || '멤버') : (p.guest_name || '게스트');
    const sub = isMember ? (p.members?.batch || '') : (p.guest_phone || '');
    ...
    const statusDot = approved
      ? '<span style="width:8px;height:8px;border-radius:50%;background:#00b386;flex-shrink:0;"></span>'
    ...
    return `<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:#fff;border:1px solid var(--border-strong);border-radius:10px;gap:8px;">
      <div style="display:flex;align-items:center;gap:10px;min-width:0;flex:1;">
        ...
        <div style="min-width:0;">
          <div style="font-size:14px;font-weight:700;color:var(--text-display);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${window.escapeHtml(name)}</div>
          <div style="font-size:11px;color:var(--text-secondary);">${window.escapeHtml(sub)} · ${joinLabel}${!approved ? ' · 대기 중' : ''}</div>
        </div>
      </div>
      <div style="display:flex;gap:4px;flex-shrink:0;">
        ${approveBtn}
...
window.removeParticipant = function(sessionId, partId) {
  window.openCustomConfirm('참가자 삭제', null, '이 참가자를 삭제하시겠습니까?', async () => {
...
window.preRegParticipant = async function() {
  ...
  const phone = document.getElementById('preRegPhone').value.trim();
  const type = document.getElementById('preRegType').value;
  ...
  document.getElementById('preRegPhone').value = '';
  showToast('참가자가 등록되었습니다.');
  await window.fetchCuppingParticipants(sessionId);
};

// ── openCuppingLineup 패치: 참가자도 함께 로드 ──
const _origOpenCuppingLineup = window.openCuppingLineup;
window.openCuppingLineup = async function(session) {
  await _origOpenCuppingLineup(session);
