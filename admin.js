...
function formatDtWithDow(dateStr) { if(!dateStr) return "-"; try { let str=String(dateStr).replace('T',' ').split('.')[0].replace(/\+.*$/,''); let parts=str.split(' '); if(parts.length<2) return str; let [y,m,d]=parts[0].split('-'); let [hh,mm]=parts[1].split(':'); if(!y||!m||!d||!hh||!mm) return str; let dow=['일','월','화','수','목','금','토'][new Date(y,m-1,d).getDay()]||''; let h=parseInt(hh),mi=parseInt(mm); let ap=h>=12?'오후':'오전'; let h12=h%12||12; let miStr=mi>0?` ${mi}분`:''; return `${y}년 ${parseInt(m)}월 ${parseInt(d)}일(${dow}) ${ap} ${h12}시${miStr}`; } catch(e) { return String(dateStr); } }
function formatDtKorean(dateStr) { if(!dateStr) return "-"; try { let str=String(dateStr).replace('T',' ').split('.')[0].replace(/\+.*$/,''); let parts=str.split(' '); if(parts.length<2) return str; let [y,m,d]=parts[0].split('-'); let [hh,mm]=parts[1].split(':'); if(!y||!m||!d||!hh||!mm) return str; let dow=['일','월','화','수','목','금','토'][new Date(y,m-1,d).getDay()]||''; let h=parseInt(hh),mi=parseInt(mm); let ap=h>=12?'오후':'오전'; let h12=h%12||12; let miStr=mi>0?` ${mi}분`:''; return `${y}년 ${parseInt(m)}월 ${parseInt(d)}일(${dow}) ${ap} ${h12}시${miStr}`; } catch(e) { return String(dateStr); } }
function formatDt(dateStr) { if(!dateStr) return "-"; try { let str=String(dateStr).replace('T',' ').split('.')[0].replace(/\+.*$/,''); let parts=str.split(' '); if(parts.length<2) return str; let [y,m,d]=parts[0].split('-'); let [hh,mm]=parts[1].split(':'); if(!y||!m||!d||!hh||!mm) return str; let dow=['일','월','화','수','목','금','토'][new Date(y,m-1,d).getDay()]||''; let h=parseInt(hh),mi=String(parseInt(mm)).padStart(2,'0'); let ap=h>=12?'오후':'오전'; let h12=h%12||12; let yearPrefix=String(y)===String(new Date().getFullYear())?'':`${y}년 `; return `${yearPrefix}${parseInt(m)}/${parseInt(d)}(${dow}) ${ap} ${h12}:${mi}`; } catch(e) { return String(dateStr); } }
...
window.renderAppTable = function(data) {
    ...
    let statusConfig=[
        ...
        {key:'완료',label:'상담 완료',color:'#7b1fa2',items:groups['상담 완료']||[]},
    ...
    statusConfig.forEach((cfg,gi)=>{
        let isOpen=cfg.items.length>0&&(cfg.key==='대기'||cfg.key==='조율'||cfg.key==='확정');
        accHtml+=`<div style="margin-bottom:12px;">
        <div onclick="let c=this.nextElementSibling;let a=this.querySelector('.acc-arrow');if(c.style.display==='none'){c.style.display='block';a.textContent='▲';}else{c.style.display='none';a.textContent='▼';}" style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:#f9fafb;border:1px solid var(--border-strong);border-radius:10px;cursor:pointer;transition:0.15s;" onmouseover="this.style.background='#f2f4f6'" onmouseout="this.style.background='#f9fafb'">
                ...
                accHtml+=`<div style="background:#fff;border:1px solid var(--border-strong);border-left:3px solid ${cfg.color};border-radius:10px;padding:14px 18px;margin-bottom:6px;cursor:pointer;transition:all 0.12s;" onmouseover="this.style.boxShadow='0 2px 8px rgba(0,0,0,0.05)'" onmouseout="this.style.boxShadow='none'" onclick="window.openCrmModal('${row.id}')">
                    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;">
                        <div style="flex:1;min-width:0;">
                            ...
                            <div style="font-size:13px;color:var(--text-secondary);line-height:1.6;display:flex;align-items:center;flex-wrap:wrap;gap:3px;">
