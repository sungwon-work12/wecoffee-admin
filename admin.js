...
window.handleNoticeMediaUpload=async function(arg){var BUCKET="notice-media";var input=null;if(arg&&arg.tagName==="INPUT")input=arg;else if(arg&&arg.target&&arg.target.tagName==="INPUT")input=arg.target;if(!input)input=document.querySelector('#noticeModal input[type="file"]')||document.querySelector('input[type="file"][accept*="image"]');var file=input&&input.files&&input.files[0];if(!file)return;var isVideo=/^video\//.test(file.type);var maxMB=isVideo?50:10;if(file.size>maxMB*1024*1024){showToast((isVideo?"영상은 ":"이미지는 ")+maxMB+"MB 이하만 올릴 수 있어요.");if(input)input.value="";return;}showToast((isVideo?"영상":"이미지")+" 업로드 중…");try{var ext=(String(file.name).split(".").pop()||(isVideo?"mp4":"jpg")).toLowerCase();var path="notices/"+Date.now()+"_"+Math.random().toString(36).slice(2,8)+"."+ext;var up=await supabaseClient.storage.from(BUCKET).upload(path,file,{cacheControl:"3600",upsert:false,contentType:file.type});if(up.error)throw up.error;var url=supabaseClient.storage.from(BUCKET).getPublicUrl(path).data.publicUrl;if(quillEditor){var range=quillEditor.getSelection(true)||{index:quillEditor.getLength()};quillEditor.insertEmbed(range.index,isVideo?"video":"image",url,"user");quillEditor.setSelection(range.index+1);}else{showToast("에디터가 준비되지 않았어요. 다시 시도해줘.");}showToast((isVideo?"영상":"이미지")+"이 첨부되었습니다.");}catch(e){console.error("[notice] 업로드 실패",e);showToast("업로드 실패: "+(e.message||"스토리지 설정 확인 필요"));}finally{if(input)input.value="";}};
...
(function () {
  ...
  var RV_KEYS = ["int_fragrance","int_aroma","int_flavor","int_aftertaste","int_acidity","int_sweetness","int_mouthfeel"];
  var RV_LABS = ["프래그런스","아로마","향미","뒷맛","산미","단맛","마우스필"];
  var STAR = String.fromCharCode(42);   // 복사 중 별표(*) 씹힘 방지
  // 센터별 공간·장비 구성(스케줄 등록과 동일 출처)
  ...
  function resolveZone(center, s){
    ...
    var i, j;
    for(i=0;i<list.length;i++){ if(s.indexOf(list[i].zone)>=0) return list[i].zone; }
    for(i=0;i<list.length;i++){ for(j=0;j<list[i].equips.length;j++){ if(list[i].equips[j] && s.indexOf(list[i].equips[j])>=0) return list[i].zone; } }
  ...
  function __wcMDStyle(){
    ...
    st.textContent=
      ...
      ".wcMD-kpi .k{flex:1;background:#fff;border:1px solid #e5e8eb;border-radius:13px;padding:13px 4px;text-align:center;box-shadow:0 1px 2px rgba(23,31,40,.05);}"+
      ".wcMD-kpi .n{font-size:18px;font-weight:800;letter-spacing:-.5px;}"+
      ".wcMD-kpi .n.hl{color:#ff7900;}"+
      ".wcMD-kpi .l{font-size:10.5px;font-weight:600;color:#8b95a1;margin-top:3px;}"+
  ...
  function __wcRailHTML(name, m){
    ...
    var kpi='<div class="wcMD-kpi"><div class="k"><div class="n">'+(m.resCnt||0)+'</div><div class="l">센터 예약</div></div>'+
      '<div class="k"><div class="n">'+(m.classCnt||0)+'</div><div class="l">수업 이수</div></div>'+
      '<div class="k"><div class="n hl">'+(m.trainCnt||0)+'</div><div class="l">훈련 참여</div></div></div>';
    var ring = rate==null ? '' :
  ...
  /* ── 멤버페이지 커핑 리뷰 모듈 연동용 shim ──
     ...
     · 모듈이 로드 시 fDat/rRes/renderTrainings/loadMoreTrn 를 래핑하는데(멤버페이지 함수),
       관리자엔 없어서 무한 재시도 루프가 돎 → 빈 함수로 스텁 처리해 루프 차단. */
  try {
    if (!window.supabaseClient && typeof supabase !== "undefined" && supabase.createClient) {
      window.supabaseClient = supabase.createClient("https://dqvzowmhxorxhiqoibmk.supabase.co", "sb_publishable_DSi3rGnuQhy6OtML_3ukEA_7ptfaoK-");
    }
  } catch (e) { console.warn("[cupping] window.supabaseClient 노출 실패", e); }
  ["fDat","rRes","renderTrainings","loadMoreTrn"].forEach(function (n) { if (typeof window[n] !== "function") window[n] = function () {}; });
  ...
  window.openHistoryModal = async function (phone, name) {
    ...
    var content = modal ? modal.querySelector(".modal-content") : null;
    var spin = null;
    // 재구성 중 '이전(좁은) 모달'이 잠깐 보이는 깜빡임 방지: content 숨기고 스피너 표시
    if (content) {
      content.style.visibility = "hidden";
      try {
        spin = document.createElement("div");
        spin.id = "wcHistSpin";
        spin.style.cssText = "position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:5;pointer-events:none;";
        spin.innerHTML = '<div style="width:34px;height:34px;border:3px solid #e5e8eb;border-top-color:#ff7900;border-radius:50%;animation:wcSpin .7s linear infinite;"></div>';
        if (!document.getElementById("wcSpinKf")) { var kf = document.createElement("style"); kf.id = "wcSpinKf"; kf.textContent = "@keyframes wcSpin{to{transform:rotate(360deg)}}"; document.head.appendChild(kf); }
  ...
  async function appendExtras(phone, name) {
    ...
    try {
      if (last4.length >= 3) {
        var rq = await supabaseClient.from("reservations").select("id,phone,status,res_date,res_time,space_equip,center").ilike("phone", "%" + last4);
        ress = (rq.data || []).filter(function (r) { return same(r.phone, phone) && !String(r.status||"").includes("취소"); });
        var tq = await supabaseClient.from("trainings").select("id,phone,status,content,created_at,name").ilike("phone", "%" + last4);
    ...
    // ── 커핑(센서리) 성장 ── (참가자는 실제 전화번호로 매칭: members(phone) 조인)
    var sessions = [];
    {
      try {
        var partsMap = {};
        ...
        parts.forEach(function (p) {
          ...
          sessions.push({ sessionId: p.session_id, title: s.title || "커핑 세션", date: s.scheduled_at, score: avgScore, acc: acc });
        });
    ...
    // (좌측 레일 KPI와 중복되므로 우측 상단 stat trio는 표기하지 않음)
    ...
    if (!usedCenters.length) h += emptyBox("공간 이용 데이터가 없습니다.");
      ...
      usedCenters.sort(function (a, b) { return (centerCnt[b] || 0) - (centerCnt[a] || 0); }).forEach(function (ck) {
        var counts = zoneByCenter[ck], cfg = WC_CENTER_SPACES[ck], zoneOrder = cfg ? cfg.map(function (z) { return z.zone; }) : [];
    ...
    if (!cupCnt) h += emptyBox("커핑 평가 데이터가 없습니다.");
      ...
      sessions.slice().reverse().forEach(function (s) {
        ...
        var sidj = String(s.sessionId || "").replace(/'/g, "");
        h += '<div class="memCupCard" style="border:1px solid var(--border-strong,#e5e8eb);border-radius:12px;padding:12px 14px;margin-bottom:8px;">' +
          '<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">' +
            ...
            '<button type="button" class="btn-outline btn-sm" style="height:30px;padding:0 10px;" onclick="window.memCupReview(\'' + sidj + '\',\'' + pj + '\',\'' + nj + '\',this)">상세</button></div>' +
          '</div>' +
          '<div class="memCupEval" style="display:none;margin-top:12px;padding-top:12px;border-top:1px solid var(--border,#f2f4f6);"></div>' +
        '</div>';
      });
    }
    // 콘텐츠 참여 이력
    ...
    try {
      ...
      if (!__mem) {
        try { var __mq = await supabaseClient.from("members").select("*").ilike("phone", "%" + last4);
          __mem = (__mq.data || []).find(function (mm) { return same(mm.phone, phone); }) || null; } catch (e) { /* noop */ }
      }
      // 결제 합계 + 최초 결제일(가입일 폴백)
      var __payTotal = 0, __firstHist = "";
      try {
        var __hh = await supabaseClient.from("member_history").select("amount,created_at").eq("member_phone", phone);
        (__hh.data || []).forEach(function (x) {
          __payTotal += (parseInt(String(x.amount || "").replace(/[^0-9]/g, "")) || 0);
          var d = x.created_at ? String(x.created_at).slice(0, 10) : "";
      ...
      var __payNodes = [];
      Array.prototype.slice.call(body.childNodes).forEach(function (n) { if (n !== host) __payNodes.push(n); });
      // 활동 pane: host만
  ...
  /* ── 코칭 코멘트(coach_notes) 공용: 로드 / 추가 / 삭제 · scope 5종 ──
       ...
       bean        → 세션 내 특정 원두(커핑 상세 · 원두별)         키: session_id + bean_id
       reservation → 센터 예약건(예약 이력 항목)                  키: ref_id(=예약 id)
       training    → 콘텐츠/훈련건(콘텐츠 이력 항목)              키: ref_id(=훈련 id)
  ...
  window.memCoachRefresh = async function (scope, phone, sessionId, beanId, refId) {
    var el = document.getElementById("cnL_" + cnKey(scope, sessionId, beanId, refId)); if (!el) return;
    try {
  ...
  window.memCoachAdd = async function (scope, phone, name, sessionId, beanId, refId) {
    ...
    var ta = document.getElementById("cnT_" + key); if (!ta) return;
    ...
    try {
      ...
      if (typeof showToast === "function") showToast(/unauthorized/i.test(e && e.message || "") ? "로그인 세션이 만료됐어요. 다시 로그인해 주세요." : "코멘트 등록에 실패했어요.");
    } finally { ta.disabled = false; }
  };
  window.memCoachDelete = async function (id, scope, phone, sessionId, beanId, refId) {
    try {
      var res = await supabaseClient.rpc("coach_note_delete", { p_id: id });
      if (res.error) throw res.error;
      await window.memCoachRefresh(scope, phone, sessionId, beanId, refId);
  ...
  window.memCoachVis = async function (id, cur, scope, phone, sessionId, beanId, refId) {
    try {
      var res = await supabaseClient.rpc("coach_note_set_visible", { p_id: id, p_visible: !cur });
      if (res.error) throw res.error;
      await window.memCoachRefresh(scope, phone, sessionId, beanId, refId);
  ...
  function cmtItem(title, dateStr, scope, phone, name, ctx, lbl){
    ...
    return '<div data-cmt-item="1" data-label="' + esc(label) + '" style="border:1px solid #eef0f3;border-radius:10px;margin-bottom:6px;overflow:hidden;">' +
      '<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;">' +
        ...
        '<button type="button" onclick="window.memCoachToggle(this,\'' + scope + '\',\'' + pj + '\',\'' + nj + '\',\'' + sj + '\',\'' + bj + '\',\'' + rj + '\')" style="border:1px solid var(--border-strong,#e5e8eb);background:#fff;border-radius:7px;font-size:11px;font-weight:700;color:#4e5968;padding:4px 9px;cursor:pointer;flex-shrink:0;">코멘트</button></div>' +
      '</div>' +
      '<div class="cnSlot" style="display:none;padding:0 12px 12px;"></div>' +
    '</div>';
  }
  window.memCoachToggle = function (btn, scope, phone, name, sessionId, beanId, refId) {
    var root = btn.closest("[data-cmt-item]"); if (!root) return;
    var slot = root.querySelector(".cnSlot"); if (!slot) return;
  ...
  function _wcReturnOly() {
    var oly = document.getElementById("wcCupHistOly");
    if (oly) {
      oly.classList.remove("wc-inline");
      oly.classList.remove("show");
      if (oly.parentNode !== document.body) document.body.appendChild(oly);
    }
  }
  window.memCupReview = async function (sessionId, phone, name, btn) {
    var card = btn && btn.closest ? btn.closest(".memCupCard") : null;
    var area = card ? card.querySelector(".memCupEval") : null;
    if (!area) return;
    // 이미 열려 있으면 닫기 (오버레이는 body로 반환)
    if (area.getAttribute("data-open") === "1") {
      _wcReturnOly();
      area.style.display = "none";
      area.removeAttribute("data-open");
      area.innerHTML = "";
      btn.textContent = "상세";
      return;
    }
    if (typeof window.wcOpenCupHistory !== "function") {
      if (typeof showToast === "function") showToast("커핑 리뷰 모듈이 로드되지 않았습니다. 관리자 페이지에 리뷰 모듈 임베드를 추가해주세요.");
      console.warn("[cupping] window.wcOpenCupHistory 없음 — 멤버페이지 커핑 리뷰 모듈 임베드 필요");
      return;
    }
    if (!sessionId) { if (typeof showToast === "function") showToast("세션 정보를 찾을 수 없습니다."); return; }
    // 다른 카드에서 열려있던 상세는 닫고 오버레이를 회수
    (function () {
      var openedArea = document.querySelector('.memCupEval[data-open="1"]');
      if (openedArea && openedArea !== area) {
        _wcReturnOly();
        openedArea.style.display = "none";
        openedArea.removeAttribute("data-open");
        openedArea.innerHTML = "";
        var oc = openedArea.closest(".memCupCard");
        var ob = oc ? oc.querySelector("button.btn-outline") : null;
        if (ob) ob.textContent = "상세";
      }
    })();
    area.innerHTML = '<div style="padding:14px;text-align:center;color:#8b95a1;font-size:13px;">불러오는 중…</div>';
    area.style.display = "block";
    area.setAttribute("data-open", "1");
    btn.textContent = "닫기";
    try {
      var sq = await supabaseClient.from("cupping_sessions").select("block_id,title,scheduled_at").eq("id", sessionId).maybeSingle();
      var sess = sq.data;
      if (!sess) { area.innerHTML = '<div style="padding:14px;text-align:center;color:#8b95a1;font-size:13px;">세션 정보를 찾을 수 없습니다.</div>'; return; }
      var content = "";
      if (sess.block_id) {
        var bq = await supabaseClient.from("blocks").select("block_date,start_time,end_time,center,category,reason").eq("id", sess.block_id).maybeSingle();
        var b = bq.data;
        if (b) content = b.block_date + " ||  || " + b.start_time + "~" + b.end_time + " || " + b.center + " || [" + b.category + "] " + b.reason;
      }
      // 리뷰 모듈은 window.uDat(로그인 멤버)를 기준으로 조회 → 대상 멤버로 세팅
      window.uDat = { phone: phone, name: name, batch: "" };
      // 모듈은 #wcCupHistBody(=#wcCupHistOly 내부)에 렌더함. 렌더 완료 후 오버레이 통째로 상세 자리로 이동.
      await window.wcOpenCupHistory(content, { content: content });
      var oly = document.getElementById("wcCupHistOly");
      if (oly) {
        oly.classList.remove("show");
        oly.classList.add("wc-inline");
        area.innerHTML = "";
        area.appendChild(oly);
        ...
        // 원두별 코멘트 (이 세션의 원두 각각)
        try {
          var bq2 = await supabaseClient.from("cupping_beans").select("id,name,sort_order").eq("session_id", sessionId).order("sort_order", { ascending: true });
          var beans = bq2.data || [];
          if (beans.length) {
            var bWrap = document.createElement("div");
            bWrap.style.cssText = "margin-top:16px;";
            bWrap.innerHTML = '<div style="font-size:13px;font-weight:800;color:#191f28;margin-bottom:10px;">원두별 코멘트</div>' +
              beans.map(function (bn) { return cmtItem(bn.name || "원두", "", "bean", phone, name, { sessionId: sessionId, beanId: bn.id }); }).join("");
            area.appendChild(bWrap);
          }
        } catch (e) { console.warn("[coach] 원두 로드 실패", e); }
      } else {
        area.innerHTML = '<div style="padding:14px;text-align:center;color:#8b95a1;font-size:13px;">리뷰를 표시할 수 없습니다.</div>';
      }
    } catch (e) {
      console.warn("[cupping] 리뷰 열기 실패", e);
      _wcReturnOly();
  ...
  function moreList(id, items, initial, renderItem) {
    ...
    return head + '<div id="' + id + '" style="display:none;">' + restItems.map(renderItem).join("") + '</div><button type="button" class="btn-outline btn-sm" id="' + id + 'Btn" style="width:100%;height:34px;margin:2px 0 6px;" onclick="window.memToggleMore(\'' + id + '\')">더보기 (' + restItems.length + '건)</button>';
  }
  ...
  function trnTitle(t) {
    ...
    return String(title).trim() || "수업·훈련 참여";
  }
  function stat(l, v) {
...
/* ═══ WeCoffee Admin · 커핑 11 — 멤버페이지 커핑 리뷰 모듈 이식 (admin.js 내장) ═══ */
/*   상세(memCupReview)가 여는 wcOpenCupHistory 정의. 관리자에선 모달 카드로 표시. */
(function(){
  "use strict";
  try{
    if(!window.supabaseClient && typeof supabase!=="undefined" && supabase.createClient){
  ...
  }catch(e){ console.warn("[cupping-shim] supabaseClient 노출 실패", e); }
  ["fDat","rRes","renderTrainings","loadMoreTrn"].forEach(function(n){ if(typeof window[n]!=="function") window[n]=function(){}; });
  if(typeof window.cAl!=="function") window.cAl=function(t,m){ if(typeof window.showToast==="function") window.showToast(String(t||"")+(m?" · "+m:"")); };
  if(typeof window.shw!=="function") window.shw=function(){};
  if(typeof window.hid!=="function") window.hid=function(){};
})();
!function(){"use strict";var e=function(){return window.supabaseClient};!function(){if(!document.getElementById("wcCupHistFootStyle")){var e=document.createElement("style");e.id="wcCupHistFootStyle",e.textContent=[".wc-cuphist-foot{display:flex;align-self:stretch;align-items:center;gap:12px;margin:18px -24px -24px;padding:16px 24px 18px;border:none;border-top:1px solid #f0f2f4;border-radius:0 0 16px 16px;background:#fff;cursor:pointer;font-family:inherit;transition:background .15s;}",".wc-cuphist-foot:active{background:#f9fafb;}",".wc-cuphist-ico{width:40px;height:40px;border-radius:12px;background:var(--primary-light,#FFF6EF);display:flex;align-items:center;justify-content:center;flex-shrink:0;}",".wc-cuphist-ico svg{width:21px;height:21px;color:var(--primary,#ff7900);}",".wc-cuphist-tx{flex:1;min-width:0;text-align:left;}",".wc-cuphist-tx .t1{font-size:14px;font-weight:700;color:#191f28;}",".wc-cuphist-tx .t2{font-size:12px;font-weight:600;color:#8b95a1;margin-top:2px;}",".wc-cuphist-chev{color:#c4ccd4;font-size:22px;font-weight:600;flex-shrink:0;}"].join(""),document.head.appendChild(e)}}();var t=[{key:"fragrance",label:"프래그런스",col:"int_fragrance"},{key:"aroma",label:"아로마",col:"int_aroma"},{key:"flavor",label:"향미",col:"int_flavor"},{key:"aftertaste",label:"뒷맛",col:"int_aftertaste"},{key:"acidity",label:"산미",col:"int_acidity"},{key:"sweetness",label:"단맛",col:"int_sweetness"},{key:"mouthfeel",label:"마우스필",col:"int_mouthfeel"}];function n(e){return String(null==e?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function i(e){return null==e||""===e?null:Number(e)}function o(e){return null==e?"—":Number(e).toFixed(1)}function r(e){var t={},n=[];return(e||[]).forEach(function(e){var i=String(null==e?"":e).trim();i&&!t[i]&&(t[i]=1,n.push(i))}),n}function a(e){if(!e)return"";if("function"==typeof window.kD){var t=window.kD(e);if(t)return String(t).replace(" (","(")}var n=new Date(e);if(isNaN(n))return String(e);var i=["일","월","화","수","목","금","토"][n.getDay()];return n.getFullYear()+"."+String(n.getMonth()+1).padStart(2,"0")+"."+String(n.getDate()).padStart(2,"0")+" ("+i+")"}function c(e){var t=new Date(e);return isNaN(t)?"":t.getFullYear()+". "+String(t.getMonth()+1).padStart(2,"0")+". "+String(t.getDate()).padStart(2,"0")}var l=null;function s(e){var t=String(e&&e.content||""),n=t.split(" || "),i=n[4]&&n[4].match(/\[(.*?)\]/)?n[4].match(/\[(.*?)\]/)[1]:"";if(t.indexOf("커핑")>=0||i.indexOf("커핑")>=0)return!0;var o,r=(o=String(t||"").split(" || ").map(function(e){return e.trim()})).length<5?null:o[0]+"|"+o[2]+"|"+o[3]+"|"+o[4];return!!(r&&l&&l.has(r))}function d(e){var t=String(e&&e.content||"").split(" || ")[4]||"",n=t.match(/^\[(.*?)\]\s*(.*)$/);return n&&n[2]||t}var p=null,f="https://cdn.prod.website-files.com/6a5efbccd4af087f960c9251/6a7c4c16d6c38f154d0a41ee_ee6a26608ebbf6f6e492819da672abbb_CVA%20%E1%84%80%E1%85%A7%E1%86%AF%E1%84%92%E1%85%A1%E1%86%B8%E1%84%91%E1%85%A7%E1%86%BC%E1%84%80%E1%85%A1.png",u={fragrance:19.58,aroma:21.77,flavor:35.14,aftertaste:37.33,acidity:57.4,sweetness:65.4,mouthfeel:73.41},w={fragrance:19.58,aroma:21.77,flavor:35.14,aftertaste:37.33,acidity:57.4,sweetness:65.4,mouthfeel:73.41,overall:83.84},h={"꽃":24.05,"과일":25.05,"신·발효":26.15,"녹색채소·식물성":27.3,"기타":28.4,"구운":29.45,"견과류·코코아":30.5,"향신료":31.6,"달콤한":32.6},g={"꽃":39.6,"과일":40.6,"신·발효":41.7,"녹색채소·식물성":42.6,"기타":43.7,"나무같은":44.7,"구운":45.8,"견과류·코코아":46.85,"향신료":47.8,"달콤한":48.7},x={"짠맛":51.1,"신맛":52.1,"단맛":53.1,"쓴맛":54.1,"감칠맛":55.2},m={"부드러운":77.8,"거친":75.6,"기름진":76.7,"입안마름":78.9,"금속성":79.9},v={"곰팡이":[84.06,93.9],"페놀":[89.34,93.9],"감자":[84.06,95]};function b(e,t){var i=(e=e||{}).intens||{},o=e.qual||{},r=t||Math.min(760,(window.innerWidth||800)-56);r<320&&(r=320);var a=3509*r/2482,c=r/100,l=document.createElement("div");l.className="wc-sheet",l.style.cssText="position:relative;width:"+r+"px;height:"+a+"px;background:#fff;margin:0 auto;";var s=document.createElement("img");function d(e,t,n,i){var o=document.createElement("div");return o.style.cssText="position:absolute;left:"+t+"%;top:"+n+"%;line-height:1;"+(i?"transform:"+i+";":""),o.innerHTML=e,l.appendChild(o),o}function p(e,t,i){null!=i&&""!==i&&d('<div style="font-size:'+1.55*c+'px;font-weight:700;color:#191f28;white-space:nowrap;">'+n(i)+"</div>",e,t,"translateY(-100%)")}function b(e,t){d('<div style="width:'+1.9*c+"px;height:"+1.9*c+'px;border-radius:3px;background:#ff7900;display:flex;align-items:center;justify-content:center;"><svg width="'+1.3*c+'" height="'+1.3*c+'" viewBox="0 0 10 10"><path d="M1.5 5 L4 7.5 L8.5 2" stroke="#fff" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg></div>',e,t,"translate(-50%,-50%)")}function y(e,t,i){i&&d('<div style="font-size:'+1.4*c+"px;font-weight:600;color:#191f28;line-height:1.5;width:"+22*c+'px;word-break:keep-all;overflow-wrap:break-word;">'+n(i)+"</div>",e,t)}s.setAttribute("crossorigin","anonymous"),s.onerror=function(){s.getAttribute("crossorigin")&&(s.removeAttribute("crossorigin"),s.src=f)},s.src=f,s.style.cssText="position:absolute;left:0;top:0;width:"+r+"px;height:"+a+"px;display:block;",s.addEventListener("load",function(){if(s.naturalWidth>0&&s.naturalHeight>0){var e=r*s.naturalHeight/s.naturalWidth;l.style.height=e+"px",s.style.height=e+"px"}}),l.appendChild(s),p(13.5,11.3,e.name),p(51.5,11.3,e.date),p(13.5,13.4,e.purpose||"센서리 훈련"),d('<div style="font-size:'+1*c+"px;font-weight:700;color:#191f28;line-height:1.12;width:"+10.3*c+'px;word-break:keep-all;overflow-wrap:break-word;">'+n(e.sample||"")+"</div>",51.5,13.55,"translateY(-100%)"),Object.keys(u).forEach(function(e){var t=i[e];null!=t&&d('<div style="background:#ff7900;color:#fff;font-size:'+1.5*c+"px;font-weight:700;line-height:1;padding:"+.5*c+"px "+.75*c+"px;border-radius:"+.9*c+"px;white-space:nowrap;box-shadow:0 0 0 "+.28*c+'px #fff;">'+Number(t).toFixed(1)+"</div>",function(e){return 16.52+e/15*45.65}(t),u[e],"translate(-50%,-50%)")}),Object.keys(w).forEach(function(e){var t=o[e];null!=t&&(d('<div style="width:'+2.7*c+"px;height:"+2.7*c+"px;border-radius:50%;background:#ff7900;color:#fff;font-size:"+1.7*c+'px;font-weight:700;display:flex;align-items:center;justify-content:center;">'+t+"</div>",67.18+2.459*(t-1),w[e],"translate(-50%,-50%)"),d('<span style="font-size:'+1.85*c+'px;font-weight:700;color:#ff7900;">'+t+"</span>",91.8,w[e],"translate(-50%,-50%)"))}),(e.descAroma||[]).forEach(function(e){null!=h[e]&&b(6.81,h[e])}),(e.descFlavor||[]).forEach(function(e){null!=g[e]&&b(6.81,g[e])}),(e.mainTastes||[]).forEach(function(e){null!=x[e]&&b(6.81,x[e])}),(e.mouthfeelDesc||[]).forEach(function(e){null!=m[e]&&b(6.81,m[e])}),y(38.5,25.5,e.aromaNotes),y(38.5,41.2,e.flavorNotes);var k=e.qnotes||{};function _(e,t){t&&d('<div style="font-size:'+1.35*c+"px;font-weight:600;color:#191f28;line-height:1.5;width:"+27*c+'px;word-break:keep-all;overflow-wrap:break-word;">'+n(t)+"</div>",66.3,e)}_(24.8,k.aroma),_(40.3,k.flavor),_(60.2,k.acidity),_(68.2,k.sweetness),_(76.2,k.mouthfeel),_(86.7,k.overall);for(var C=0;C<(e.nonuniform||0)&&C<5;C++)b(76.1+1.29*C,92.09);for(var E=0;E<(e.defective||0)&&E<5;E++)b(76.1+1.29*E,93.18);(e.defectTypes||[]).forEach(function(e){v[e]&&b(v[e][0],v[e][1])});var z="";return null!=e.cva&&(z+='<div style="font-size:'+2*c+"px;font-weight:700;color:#ff7900;margin-bottom:"+.7*c+'px;">CVA 커핑 점수 '+Number(e.cva).toFixed(1)+' <span style="font-size:'+1.3*c+'px;color:#8b95a1;font-weight:700;">/ 100</span></div>'),e.extrinsic&&(z+='<div style="font-size:'+1.4*c+'px;font-weight:600;color:#191f28;line-height:1.55;word-break:keep-all;overflow-wrap:break-word;">'+n(e.extrinsic)+"</div>"),z&&d('<div style="width:'+50*c+'px;">'+z+"</div>",8,84.4),l}var y=null;function k(e){"function"==typeof window.showToast&&window.showToast(e)}function _(e,t,n){if(!function(){try{if(window.matchMedia&&window.matchMedia("(any-pointer: coarse)").matches)return!0}catch(e){}return/iPhone|iPad|iPod|Android/i.test(navigator.userAgent||"")}())try{var i=e?URL.createObjectURL(e):t,o=document.createElement("a");return o.download=n,o.href=i,document.body.appendChild(o),o.click(),o.remove(),e&&setTimeout(function(){try{URL.revokeObjectURL(i)}catch(e){}},4e3),void k("이미지를 저장했어요")}catch(e){}!function(e,t,n){var i=document.getElementById("wcExpOv");i||((i=document.createElement("div")).id="wcExpOv",i.style.cssText="position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,.86);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:20px;box-sizing:border-box;"),document.body.appendChild(i),i.innerHTML="";var o=document.createElement("img");o.src=t||(e?URL.createObjectURL(e):""),o.alt=n,o.style.cssText="max-width:100%;max-height:68vh;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,.45);-webkit-touch-callout:default;";var r=document.createElement("div");r.innerHTML="이미지를 <b>길게 눌러</b> ‘사진에 저장’을 선택하세요",r.style.cssText="color:#fff;font-size:14px;font-weight:700;margin-top:16px;text-align:center;letter-spacing:-.3px;line-height:1.5;";var a=document.createElement("div");a.style.cssText="display:flex;gap:8px;margin-top:16px;";var c=!1;try{c=!!(e&&navigator.canShare&&navigator.canShare({files:[new File([e],n,{type:"image/png"})]}))}catch(e){}if(c){var l=document.createElement("button");l.textContent="저장·공유",l.style.cssText="padding:13px 22px;border:none;border-radius:12px;background:#ff7900;color:#fff;font-size:15px;font-weight:700;cursor:pointer;",l.onclick=function(){try{navigator.share({files:[new File([e],n,{type:"image/png"})],title:"커핑 결과"})}catch(e){}},a.appendChild(l)}var s=document.createElement("button");s.textContent="닫기",s.style.cssText="padding:13px 22px;border:none;border-radius:12px;background:#fff;color:#191f28;font-size:15px;font-weight:700;cursor:pointer;",s.onclick=function(){try{i.remove()}catch(e){}},a.appendChild(s),i.appendChild(o),i.appendChild(r),i.appendChild(a)}(e,t,n)}function C(){if(!document.getElementById("wcCupHistOly")){var e=document.createElement("style");e.textContent=[".wc-cuphist-foot{display:flex;align-self:stretch;align-items:center;gap:12px;margin:18px -24px -24px;padding:16px 24px 18px;border:none;border-top:1px solid #f0f2f4;border-radius:0 0 16px 16px;background:#fff;cursor:pointer;font-family:inherit;transition:background .15s;}",".wc-cuphist-foot:active{background:#f9fafb;}",".wc-cuphist-ico{width:40px;height:40px;border-radius:12px;background:var(--primary-light,#FFF6EF);display:flex;align-items:center;justify-content:center;flex-shrink:0;}",".wc-cuphist-ico svg{width:21px;height:21px;color:var(--primary,#ff7900);}",".wc-cuphist-tx{flex:1;min-width:0;text-align:left;}",".wc-cuphist-tx .t1{font-size:14px;font-weight:700;color:#191f28;}",".wc-cuphist-tx .t2{font-size:12px;font-weight:600;color:#8b95a1;margin-top:2px;}",".wc-cuphist-chev{color:#c4ccd4;font-size:22px;font-weight:600;flex-shrink:0;}","#wcCupHistOly .wc-ch-wrap{display:flex;flex-direction:column;height:100%;width:100%;max-width:860px;margin:auto;background:#F2F4F6;}","#wcCupHistOly .wc-ch-head{padding:16px 20px;background:#fff;display:flex;align-items:center;position:sticky;top:0;z-index:10;border-bottom:1px solid var(--border-strong,#e5e8eb);}","#wcCupHistOly .wc-ch-body{padding:18px 16px 40px;overflow-y:auto;flex:1;-webkit-overflow-scrolling:touch;}",".wc-ch-card{background:#fff;border:1px solid #e5e8eb;border-radius:14px;padding:16px 18px;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,0.02);}",".wc-ch-tabs{display:flex;flex-direction:column;align-items:stretch;gap:6px;overflow-x:visible;padding:2px 2px 8px;scrollbar-width:none;}",".wc-ch-tabs::-webkit-scrollbar{display:none;}",".wc-ch-tab{flex:0 0 auto;width:100%;text-align:left;padding:8px 14px;border-radius:10px;border:1px solid #e5e8eb;background:#fff;color:#4e5968;font-size:13px;font-weight:700;cursor:pointer;white-space:normal;word-break:keep-all;line-height:1.4;}",".wc-ch-tab.on{background:#191f28;border-color:#191f28;color:#fff;}",".wc-ch-seg{display:inline-flex;background:#f2f4f6;border-radius:10px;padding:4px;margin:2px 0 12px;}",".wc-ch-seg button{border:none;background:transparent;padding:8px 18px;font-size:13px;font-weight:700;border-radius:7px;cursor:pointer;color:#8b95a1;font-family:inherit;}",".wc-ch-seg button.on{background:#fff;color:#191f28;box-shadow:0 2px 8px rgba(0,0,0,.06);}",".wc-ch-radar{display:flex;flex-direction:column;align-items:center;padding:6px 0 4px;}",".wc-ch-legend{display:flex;gap:16px;justify-content:center;margin-top:6px;font-size:12px;font-weight:700;color:#4e5968;}",".wc-ch-legend span{display:inline-flex;align-items:center;gap:5px;}",".wc-ch-legend i{width:10px;height:10px;border-radius:3px;display:inline-block;}",".wc-ch-scorerow{display:flex;gap:8px;margin-top:8px;}",".wc-ch-sc{flex:1;background:#f9fafb;border:1px solid #eef0f3;border-radius:10px;padding:12px 8px;text-align:center;}",".wc-ch-sc .l{font-size:11px;color:#8b95a1;font-weight:600;margin-bottom:3px;}",".wc-ch-sc .v{font-size:20px;font-weight:700;color:#191f28;}",".wc-ch-sc .v small{font-size:11px;color:#8b95a1;font-weight:700;}",".wc-ch-cmp{width:100%;border-collapse:collapse;margin-top:12px;font-size:13px;}",".wc-ch-cmp th{font-size:11px;color:#8b95a1;font-weight:700;text-align:center;padding:6px 4px;border-bottom:1px solid #eef0f3;}",".wc-ch-cmp td{text-align:center;padding:8px 4px;border-bottom:1px solid #f2f4f6;font-weight:700;color:#191f28;}",".wc-ch-cmp td.lab{text-align:left;color:#4e5968;font-weight:600;}",".wc-ch-cmp td.me{color:#ea6f00;}.wc-ch-cmp td.rf{color:#3182f6;}",".wc-dev-ok{color:#00b386!important;}.wc-dev-mid{color:#e08600!important;}.wc-dev-hi{color:#e5484d!important;}",".wc-ch-chips{display:flex;flex-wrap:wrap;gap:5px;margin-top:6px;}",".wc-ch-chip{font-size:11px;font-weight:600;color:#5a6572;background:#f2f4f6;border-radius:6px;padding:3px 9px;white-space:nowrap;}",".wc-ch-qrow{display:flex;gap:8px;font-size:12.5px;line-height:1.55;margin-top:7px;}",".wc-ch-qrow .k{flex-shrink:0;font-weight:700;color:#ea6f00;min-width:42px;}",".wc-ch-qrow .t{color:#4e5968;word-break:break-word;}",".wc-ch-banner{background:#eef7ff;border:1px solid #d6e9ff;color:#2f6fb0;border-radius:10px;padding:10px 12px;font-size:12.5px;font-weight:600;margin-top:10px;text-align:center;}",".wc-ch-formhost{background:#fff;border:1px solid #e5e8eb;border-radius:14px;padding:14px;overflow-x:auto;}",".wc-ch-save{width:100%;margin-top:10px;padding:12px;border-radius:10px;border:1px solid #e5e8eb;background:#fff;color:#191f28;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;}"].join(""),document.head.appendChild(e);var t=document.createElement("div");t.id="wcCupHistOly",t.className="full-page-overlay",t.innerHTML='<div class="wc-ch-wrap"><div class="wc-ch-head"><div class="fs24 fw3 c22 cp-click" style="margin-right:16px;padding-right:8px;cursor:pointer;" onclick="window.wcCloseCupHistory()">←</div><div class="fs18 fw8 c22" style="letter-spacing:-0.5px;">커핑 내역</div></div><div class="wc-ch-body" id="wcCupHistBody"></div></div>',document.body.appendChild(t)}}function E(e){return p.recs.filter(function(t){return t.bean_id===e})[0]}function z(){var e=document.getElementById("wcCupTabs");e&&(e.innerHTML=p.recs.map(function(e,t){return'<span class="wc-ch-tab'+(e.bean_id===p.activeBean?" on":"")+'" onclick="window.wcCupTab(\''+e.bean_id+"')\">"+(t+1)+". "+n(p.beanName[e.bean_id]||"원두")+"</span>"}).join(""))}function T(){var e=E(p.activeBean);if(e){var a="basic"===e.form_type,l=document.getElementById("wcCupToggleWrap");l&&(l.innerHTML=a?"":'<div class="wc-ch-seg"><button class="'+("card"===p.view?"on":"")+'" onclick="window.wcCupToggleView(\'card\')">레이더</button><button class="'+("form"===p.view?"on":"")+'" onclick="window.wcCupToggleView(\'form\')">CVA 폼</button></div>');var s=document.getElementById("wcCupView");s&&(a||"form"!==p.view?s.innerHTML=function(e,a){var c,l,s=p.refMap[e.bean_id]||null,d=t.map(function(t){return i(e[t.col])}),f=s?t.map(function(e){return i(s[e.col])}):null,u=t.map(function(e){return e.label}),w=(l=e.bean_id,(p.others||[]).filter(function(e){return String(e.bean_id)===String(l)&&!e.is_me})),h=w.length?(c=w,t.map(function(e){var t=c.map(function(t){return i(t[e.col])}).filter(function(e){return null!=e});return t.length?t.reduce(function(e,t){return e+t},0)/t.length:null})):null;h&&h.every(function(e){return null==e})&&(h=null),window._wcChRadarFn=function(e,t,n,i){var o=(i=i||[]).length||1;function r(e,t){var n=-Math.PI/2+e*(2*Math.PI/o),i=(null==t?0:t)/15*105;return[170+i*Math.cos(n),150+i*Math.sin(n)]}function a(e){return e.map(function(e,t){return r(t,e).join(",")}).join(" ")}var c="";return[5,10,15].forEach(function(e){var t=i.map(function(t,n){return r(n,e).join(",")}).join(" ");c+='<polygon points="'+t+'" fill="none" stroke="#e5e8eb" stroke-width="1"/>'}),i.forEach(function(e,t){var n=r(t,15),i=r(t,17.8);c+='<line x1="170" y1="150" x2="'+n[0]+'" y2="'+n[1]+'" stroke="#eceef1" stroke-width="1"/>';var o=Math.abs(i[0]-170)<6?"middle":i[0]<170?"end":"start";c+='<text x="'+i[0]+'" y="'+(i[1]+4)+'" font-size="11" font-weight="700" fill="#8b95a1" text-anchor="'+o+'">'+e+"</text>"}),t&&(c+='<polygon points="'+a(t)+'" fill="rgba(49,130,246,0.14)" stroke="#3182f6" stroke-width="2"/>',t.forEach(function(e,t){if(null!=e){var n=r(t,e);c+='<circle cx="'+n[0]+'" cy="'+n[1]+'" r="3" fill="#3182f6"/>'}})),n&&(c+='<polygon points="'+a(n)+'" fill="rgba(18,184,134,0.14)" stroke="#12b886" stroke-width="2" stroke-dasharray="5 3"/>',n.forEach(function(e,t){if(null!=e){var n=r(t,e);c+='<circle cx="'+n[0]+'" cy="'+n[1]+'" r="3" fill="#12b886"/>'}})),c+='<polygon points="'+a(e)+'" fill="rgba(255,121,0,0.16)" stroke="#ff7900" stroke-width="2"/>',e.forEach(function(e,t){if(null!=e){var n=r(t,e);c+='<circle cx="'+n[0]+'" cy="'+n[1]+'" r="3.5" fill="#fff" stroke="#ff7900" stroke-width="2"/>'}}),'<svg width="340" height="300" viewBox="0 0 340 300">'+c+"</svg>"},window._wcChMe=d,window._wcChRef=f,window._wcChAx=u,window._wcChAvg=h;window._wcChMyNotes=(function(){var _s={},_a=[];[].concat(e.notes_fragrance||[],e.notes_aroma||[],e.notes_tasting||[],e.notes_custom||[]).forEach(function(z){var w=String(z==null?"":z).trim();w&&!_s[w]&&(_s[w]=1,_a.push(w))});return _a})();var g=window._wcChRadarFn(d,f,h,u),x='<div class="wc-ch-card">';x+='<div class="wc-ch-radar">'+g+'<div class="wc-ch-legend"><span><i style="background:#ff7900"></i>나</span>'+(h?'<span><i style="background:#12b886"></i>참가자 평균</span>':"")+(f?'<span><i style="background:#3182f6"></i>레퍼런스</span>':"")+"</div></div>",f||(x+='<div class="wc-ch-banner">레퍼런스가 아직 없어요. 등록되면 비교가 채워져요.</div>');var m=d.filter(function(e){return null!=e}),v=m.length?m.reduce(function(e,t){return e+t},0)/m.length:null,b=null==i(e.cva_score)&&!a,y=b?v:i(e.cva_score),k=b?"내 평균 강도":a?"내 위커피 점수":"내 CVA 점수",_=b?" / 15":a?" / 120":" / 100",C="";if(f){var E=0,z=0;d.forEach(function(e,t){null!=e&&null!=f[t]&&(E+=Math.abs(e-f[t]),z++)});var T=z?E/z:null;C='<div class="wc-ch-sc"><div class="l">레퍼런스와 차이</div><div class="v '+(null==T?"":T<=1?"wc-dev-ok":T<=2.5?"wc-dev-mid":"wc-dev-hi")+'">'+(null==T?"—":T.toFixed(1))+"<small> pt</small></div></div>"}var S=a&&null!=i(e.basic_overall)?'<div style="font-size:11px;color:#ea6f00;font-weight:700;margin-top:2px;">전체적 인상 '+o(i(e.basic_overall))+"</div>":"";if(x+='<div class="wc-ch-scorerow"><div class="wc-ch-sc"><div class="l">'+k+'</div><div class="v" style="color:#ea6f00;">'+o(y)+"<small>"+_+"</small></div>"+S+"</div>"+C+"</div>",x+='<div style="font-size:15px;font-weight:700;color:#191f28;margin:24px 0 12px;">강도 비교</div>',x+='<div style="border:1px solid #f0f1f3;border-radius:16px;padding:5px 4px;background:#fff;">',t.forEach(function(e,t){var n=d[t],i=f?f[t]:null,r=(null!=i?'<div style="position:absolute;top:50%;left:'+Math.max(0,Math.min(15,i))/15*100+'%;transform:translate(-50%,-50%);width:13px;height:13px;border-radius:50%;background:#3182f6;border:2px solid #fff;z-index:1;"></div>':"")+(null!=n?'<div style="position:absolute;top:50%;left:'+Math.max(0,Math.min(15,n))/15*100+'%;transform:translate(-50%,-50%);width:15px;height:15px;border-radius:50%;background:#ff7900;border:2px solid #fff;box-shadow:0 1px 3px rgba(0,0,0,.18);z-index:2;"></div>':""),a='<span style="color:#ff7900;">'+(null!=n?o(n):"—")+"</span>"+(null!=i?'<span style="color:#c4cad2;font-weight:400;margin:0 3px;">/</span><span style="color:#3182f6;font-weight:500;">'+o(i)+"</span>":"");x+='<div style="display:flex;align-items:center;gap:14px;padding:14px 14px;'+(t>0?"border-top:1px solid #f0f1f3;":"")+'"><span style="width:64px;flex-shrink:0;font-size:12.5px;font-weight:500;color:#8b95a1;">'+e.label+'</span><div style="position:relative;flex:1;height:20px;"><div style="position:absolute;left:0;right:0;top:50%;height:5px;transform:translateY(-50%);background:#e8eaed;border-radius:99px;"></div>'+r+'</div><span style="width:70px;flex-shrink:0;text-align:right;font-size:13px;font-weight:700;">'+a+"</span></div>"}),x+="</div>",a)e.extrinsic&&String(e.extrinsic).trim()&&(x+='<div style="margin-top:14px;"><div style="font-size:12px;font-weight:700;color:#191f28;margin-bottom:6px;">내 컵노트 · 총평</div><div style="font-size:13px;color:#4e5968;line-height:1.6;word-break:keep-all;">'+n(e.extrinsic)+"</div></div>");else{var Ma=r([].concat(e.notes_fragrance||[],e.notes_aroma||[],e.notes_custom||[])),Mf=r([].concat(e.notes_tasting||[])),M=Ma.concat(Mf),H=s&&s.ref_notes?s.ref_notes:[];x+='<div style="margin-top:14px;display:flex;flex-direction:column;gap:10px;">',x+='<div><div style="font-size:12px;font-weight:700;color:#191f28;margin-bottom:2px;">내 향미 노트</div>'+(M.length?(function(){var _g=function(lb,ar){return ar.length?'<div style="font-size:11px;font-weight:700;color:#8b95a1;margin:6px 0 4px;">'+lb+'</div><div class="wc-ch-chips">'+ar.map(function(e){return'<span class="wc-ch-chip">'+n(e)+"</span>"}).join("")+"</div>":""};return _g("향",Ma)+_g("맛",Mf)})():'<div style="font-size:12px;color:#b0b8c1;margin-top:4px;">기록 없음</div>')+"</div>",H.length&&(x+='<div><div style="font-size:12px;font-weight:700;color:#3182f6;margin-bottom:2px;">레퍼런스 노트</div><div class="wc-ch-chips">'+H.map(function(e){return'<span class="wc-ch-chip" style="background:#eaf2fe;color:#3172c0;">'+n(e)+"</span>"}).join("")+"</div></div>"),x+='<div id="wcChNoteCmp" style="margin-top:2px;"></div></div>';var L=e.q_notes||{},j="";[["aroma","아로마"],["flavor","향미"],["acidity","산미"],["sweetness","단맛"],["mouthfeel","마우스필"],["overall","종합"]].forEach(function(e){L[e[0]]&&String(L[e[0]]).trim()&&(j+='<div class="wc-ch-qrow"><span class="k">'+e[1]+'</span><span class="t">'+n(L[e[0]])+"</span></div>")}),j&&(x+='<div style="margin-top:12px;border-top:1px solid #f2f4f6;padding-top:10px;"><div style="font-size:12px;font-weight:700;color:#191f28;margin-bottom:2px;">품질 인상 노트</div>'+j+"</div>"),e.extrinsic&&String(e.extrinsic).trim()&&(x+='<div class="wc-ch-qrow" style="margin-top:10px;"><span class="k" style="color:#8b95a1;">외재</span><span class="t">'+n(e.extrinsic)+"</span></div>")}if(x+="</div>",w.length){var A=w.slice().sort(function(e,t){return(i(t.cva_score)||0)-(i(e.cva_score)||0)});x+='<div style="font-size:15px;font-weight:700;color:#191f28;margin:24px 0 12px;">다른 참가자 '+w.length+"명</div>",A.forEach(function(e){var r,a=function(e){if(e.is_member){var t=e.batch?String(e.batch).replace(/[\[\]]/g,"").trim():"";return(t?t+" ":"")+(e.disp_name||"멤버")}return"게스트 "+(e.disp_name||"")}(e),c=i(e.cva_score),l=null!=c?'CVA <b style="font-weight:700;color:#191f28;">'+o(c)+"</b>":'강도 <b style="font-weight:700;color:#191f28;">'+(r=t.map(function(t){return i(e[t.col])}).filter(function(e){return null!=e}),(r.length?(r.reduce(function(e,t){return e+t},0)/r.length).toFixed(1):"—")+"</b>"),s=t.map(function(t){var n=i(e[t.col]);return'<span style="font-size:13px;color:#8b95a1;font-weight:400;white-space:nowrap;">'+t.label+'<b style="color:#191f28;font-weight:700;font-size:15px;margin-left:8px;">'+(null==n?"—":o(n))+"</b></span>"}).join("");x+='<div class="wc-ch-pcard" data-vals="'+t.map(function(t){var n=i(e[t.col]);return null==n?"":n}).join(",")+'" data-nm="'+n(a)+'" style="border:1px solid #f0f1f3;border-radius:18px;padding:18px;margin-bottom:10px;background:#fff;cursor:pointer;"><div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:16px;"><span style="display:flex;align-items:center;gap:8px;min-width:0;font-size:15px;font-weight:700;color:#191f28;"><span style="width:9px;height:9px;border-radius:50%;background:#b6bdc6;flex-shrink:0;"></span><span style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+n(a)+'</span></span><span style="flex-shrink:0;font-size:12.5px;font-weight:600;color:#4e5968;background:#f8f9fb;border:1px solid #e8eaed;padding:5px 11px;border-radius:999px;">'+l+'</span></div><div style="display:flex;flex-wrap:wrap;gap:15px 22px;">'+s+"</div>"+(function(){var _s={},F=[];[].concat(e.notes_fragrance||[],e.notes_aroma||[],e.notes_tasting||[],e.notes_custom||[]).forEach(function(z){var w=String(z==null?"":z).trim();w&&!_s[w]&&(_s[w]=1,F.push(w))});var Q=e.q_notes||{},q="";[["aroma","아로마"],["flavor","향미"],["acidity","산미"],["sweetness","단맛"],["mouthfeel","마우스필"],["overall","종합"]].forEach(function(k){Q[k[0]]&&String(Q[k[0]]).trim()&&(q+='<div class="wc-ch-qrow"><span class="k">'+k[1]+'</span><span class="t">'+n(Q[k[0]])+"</span></div>")});e.extrinsic&&String(e.extrinsic).trim()&&(q+='<div class="wc-ch-qrow" style="margin-top:8px"><span class="k" style="color:#8b95a1">총평</span><span class="t">'+n(e.extrinsic)+"</span></div>");var c=F.length?'<div class="wc-ch-chips">'+F.map(function(x){return'<span class="wc-ch-chip">'+n(x)+"</span>"}).join("")+"</div>":"";return c||q?'<div style="border-top:1px solid #f0f1f3;margin-top:4px;padding-top:12px"><div style="font-size:11.5px;font-weight:700;color:#8b95a1;margin-bottom:6px">컵노트</div>'+c+(q&&c?'<div style="margin-top:8px">'+q+"</div>":q)+"</div>":""})()+"</div>"}),h&&(x+='<div style="font-size:11.5px;color:#8b95a1;margin-top:10px;text-align:center;">레이더의 초록 점선이 참가자 평균이에요.</div>')}return x}(e,a):function(e,t){var n=p.beanName[t.bean_id]||"",o=p.refMap[t.bean_id]||null,a=!!o;function l(e,o){return{name:o?"교육 매니저":window.uDat&&window.uDat.name||"",date:c(p.sess&&p.sess.scheduled_at),purpose:p.sess&&p.sess.title||"센서리 훈련",sample:n,intens:{fragrance:i(e.int_fragrance),aroma:i(e.int_aroma),flavor:i(e.int_flavor),aftertaste:i(e.int_aftertaste),acidity:i(e.int_acidity),sweetness:i(e.int_sweetness),mouthfeel:i(e.int_mouthfeel)},qual:o?{}:{fragrance:i(t.q_fragrance),aroma:i(t.q_aroma),flavor:i(t.q_flavor),aftertaste:i(t.q_aftertaste),acidity:i(t.q_acidity),sweetness:i(t.q_sweetness),mouthfeel:i(t.q_mouthfeel),overall:i(t.q_overall)},descAroma:o?[]:t.desc_aroma||[],descFlavor:o?[]:t.desc_flavor||[],aromaNotes:o?(e.ref_notes||[]).join(", "):r([].concat(t.notes_fragrance||[],t.notes_aroma||[])).join(", "),flavorNotes:o?"":[].concat(t.notes_tasting||[],t.notes_custom||[]).join(", "),mainTastes:o?[]:t.main_tastes||[],mouthfeelDesc:o?[]:t.mouthfeel_desc||[],nonuniform:o?0:t.nonuniform_cups||0,defective:o?0:t.defective_cups||0,defectTypes:o?[]:t.defect_types||[],extrinsic:o?"":t.extrinsic||"",qnotes:o?{}:t.q_notes||{},cva:o?null:i(t.cva_score)}}var s=l(t,!1),d=a?l(o,!0):null;function f(e,t){var n=p._fm===e;return'<button type="button" data-fm="'+e+'" style="border:none;background:'+(n?"#fff":"transparent")+";color:"+(n?"#191f28":"#8b95a1")+";padding:8px 16px;font-size:13px;font-weight:600;border-radius:8px;cursor:pointer;font-family:inherit;"+(n?"box-shadow:0 1px 3px rgba(0,0,0,.1);":"")+'">'+t+"</button>"}"host"!==p._fm||a||(p._fm="mine"),"compare"!==p._fm||a||(p._fm="mine"),p._fm=p._fm||"mine";var w=a?'<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;margin-bottom:12px;"><div style="display:inline-flex;background:#f2f4f6;border-radius:10px;padding:3px;">'+f("mine","내 폼")+f("host","매니저 폼")+f("compare","비교")+'</div><div id="wcFmLegend" style="display:'+("compare"===p._fm?"flex":"none")+';gap:12px;align-items:center;font-size:12px;font-weight:600;color:#4e5968;"><span style="display:inline-flex;align-items:center;gap:5px;"><i style="width:11px;height:11px;border-radius:3px;background:#ff7900;display:inline-block;"></i>나</span><span style="display:inline-flex;align-items:center;gap:5px;"><i style="width:11px;height:11px;border-radius:3px;background:#3182f6;display:inline-block;"></i>교육 매니저</span></div></div>':"";e.innerHTML=w+'<div class="wc-ch-formhost" id="wcCupFormHost"></div>';var h=document.getElementById("wcCupFormHost");function g(){h.innerHTML="";var e=b("host"===p._fm?d:s);h.appendChild(e),"compare"===p._fm&&o&&Object.keys(u).forEach(function(t){var n=i(o["int_"+t]);if(null!=n){var r=(e.offsetWidth||340)/100,a=16.52+n/15*45.65,c=u[t],l=document.createElement("div");l.style.cssText="position:absolute;left:"+a+"%;top:"+(c-1.1)+"%;height:2.2%;border-left:"+Math.max(1.5,.42*r)+"px solid #3182f6;opacity:.75;transform:translateX(-50%);z-index:5;",e.appendChild(l);var s=document.createElement("div");s.style.cssText="position:absolute;left:"+a+"%;top:"+(c-1.5)+"%;transform:translate(-50%,-100%);background:#3182f6;color:#fff;font-size:"+1.1*r+"px;font-weight:700;line-height:1;padding:"+.3*r+"px "+.5*r+"px;border-radius:"+.6*r+"px;opacity:.95;box-shadow:0 0 0 "+.2*r+"px #fff;z-index:6;white-space:nowrap;",s.textContent=Number(n).toFixed(1),e.appendChild(s)}})}g(),e.querySelectorAll("[data-fm]").forEach(function(t){t.addEventListener("click",function(){p._fm=t.getAttribute("data-fm"),e.querySelectorAll("[data-fm]").forEach(function(e){var t=e.getAttribute("data-fm")===p._fm;e.style.background=t?"#fff":"transparent",e.style.color=t?"#191f28":"#8b95a1",e.style.boxShadow=t?"0 1px 3px rgba(0,0,0,.1)":"none"});var n=document.getElementById("wcFmLegend");n&&(n.style.display="compare"===p._fm?"flex":"none"),g()})});var x=document.createElement("button");x.type="button",x.className="wc-ch-save",x.textContent="이미지로 저장",x.onclick=function(){var e=document.querySelector("#wcCupFormHost .wc-sheet");e&&(k("이미지 생성 중…"),(window.html2canvas?Promise.resolve(window.html2canvas):y||(y=new Promise(function(e,t){var n=document.createElement("script");n.src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js",n.onload=function(){e(window.html2canvas)},n.onerror=function(){y=null,t(new Error("html2canvas load fail"))},document.head.appendChild(n)}))).then(function(t){t(e,{backgroundColor:"#ffffff",scale:2,useCORS:!0}).then(function(e){var t="";try{t=e.toDataURL("image/png")}catch(e){}e.toBlob?e.toBlob(function(e){_(e,t,"커핑결과_"+(n||"session")+".png")},"image/png"):_(null,t,"커핑결과_"+(n||"session")+".png")}).catch(function(e){console.error("[cupping] 이미지 저장 실패",e),k("이미지 저장 실패")})}).catch(function(){k("이미지 모듈을 불러오지 못했어요")}))},e.appendChild(x)}(s,e))}}function S(e,t,i,o){var r=t&&d(t)||"커핑 세션",c=a(t&&String(t.content||"").split(" || ")[0]);e.innerHTML='<div class="wc-ch-card"><div style="font-size:12px;color:#8b95a1;font-weight:600;margin-bottom:4px;">'+n(c)+'</div><div style="font-size:17px;font-weight:700;color:#191f28;margin-bottom:2px;word-break:keep-all;">'+n(r)+'</div></div><div style="padding:40px 20px;text-align:center;"><div style="font-size:15px;font-weight:700;color:#4e5968;margin-bottom:8px;word-break:keep-all;">'+n(i)+'</div><div style="font-size:13px;color:#8b95a1;line-height:1.6;word-break:keep-all;">'+n(o)+"</div></div>"}async function M(){if(window.uDat&&Array.isArray(window.uDat.historyTrainings)){var t=document.getElementById("mpTrainings");if(t){await async function(){if(l)return l;var t=new Set;try{var n=((await e().from("cupping_sessions").select("block_id")).data||[]).map(function(e){return e.block_id}).filter(Boolean);n.length&&((await e().from("blocks").select("block_date,start_time,end_time,center,category,reason").in("id",n)).data||[]).forEach(function(e){t.add(e.block_date+"|"+e.start_time+"~"+e.end_time+"|"+e.center+"|["+e.category+"] "+e.reason)})}catch(e){console.error("[cupping] 세션 시그니처 로드 실패",e)}return l=t,t}();var n=window.uDat.historyTrainings.filter(s);n.length&&t.querySelectorAll(".res-card").forEach(function(e){if(!e._wcCupHist){var t=e.textContent||"",i=n.find(function(e){var n=String(e.content||"").split(" || "),i=d(e),o="function"==typeof window.kD?window.kD(n[0]):n[0];return i&&t.indexOf(i)>=0&&(!o||t.indexOf(o)>=0)});if(i){e._wcCupHist=!0;var o=document.createElement("div");o.className="wc-cuphist-foot",o.setAttribute("role","button"),o.innerHTML='<div class="wc-cuphist-tx"><div class="t1">내 커핑 결과 보기</div><div class="t2">원두별 점수와 레퍼런스 비교를 확인해요</div></div><span class="wc-cuphist-chev">›</span>',o.addEventListener("click",function(e){e.preventDefault(),e.stopPropagation(),window.wcOpenCupHistory(i.content,i)}),e.appendChild(o)}}})}}}function H(e){var t=window[e];"function"==typeof t?t._wcCupWrapped||(window[e]=function(){var e=t.apply(this,arguments);return setTimeout(M,30),e},window[e]._wcCupWrapped=!0):setTimeout(function(){H(e)},300)}window.wcCloseCupHistory=function(){var e=document.getElementById("wcCupHistOly");e&&e.classList.remove("show")},window.wcOpenCupHistory=async function(r,c){if(window.uDat){C();var l=document.getElementById("wcCupHistOly");l.parentNode===document.body&&document.body.lastElementChild===l||document.body.appendChild(l);var s=document.getElementById("wcCupHistBody");s.innerHTML="",l.classList.add("show"),window.shw&&window.shw("ld");try{var f=await async function(t){var n=String(t||"").split(" || ").map(function(e){return e.trim()});if(n.length<5)return null;var i=n[0],o=n[2],r=n[3],a=n[4],c=((await e().from("blocks").select("*").eq("block_date",i).eq("center",r)).data||[]).find(function(e){return e.start_time+"~"+e.end_time===o&&"["+e.category+"] "+e.reason===a});return c&&(await e().from("cupping_sessions").select("id,slug,title,scheduled_at").eq("block_id",c.id).maybeSingle()).data||null}(r);if(!f)return void S(s,c,"아직 커핑 세션 데이터가 없어요.","호스트가 세션을 개설·진행한 뒤 조회할 수 있어요.");var u=await async function(t,n){var i=await e().rpc("find_my_participant",{p_session_id:String(t),p_phone:n});return i&&i.data?i.data:null}(f.id,window.uDat.phone);if(!u)return void S(s,c,"이 세션의 참가 기록을 찾지 못했어요.","세션에 참여한 경우에만 조회됩니다.");var w=(await e().rpc("records_by_participant",{p_participant_id:String(u)})).data||[],h={},g={},x=[];if(((await e().from("cupping_beans").select("id,name,sort_order").eq("session_id",f.id).order("sort_order",{ascending:!0})).data||[]).forEach(function(e,t){h[e.id]=e.name,g[e.id]=t,x.push(e.id)}),!x.length)return void S(s,c,"세션 데이터가 없어요.","원두 라인업이 아직 없어요.");var m={};((await e().from("cupping_references").select("*").in("bean_id",x)).data||[]).forEach(function(e){m[e.bean_id]=e});var v={};w.forEach(function(e){v[e.bean_id]=e});var b=x.map(function(e){return v[e]||{bean_id:e,form_type:"cva"}}),y=[];try{y=(await e().rpc("session_review_for_member",{p_session_id:String(f.id),p_phone:window.uDat.phone})).data||[]}catch(e){console.warn("[cupping] 참가자 평가 로드 실패",e)}p={recs:b,refMap:m,beanName:h,sess:f,trn:c,view:"card",activeBean:b[0]&&b[0].bean_id||null,others:y},function(e){var r=p.sess,c=r.title||d(p.trn)||"커핑 세션",l=a(r.scheduled_at||p.trn&&String(p.trn.content||"").split(" || ")[0]),s=p.recs.map(function(e){return i(e.cva_score)}).filter(function(e){return null!=e}),f=s.length?s.reduce(function(e,t){return e+t},0)/s.length:null,u=[];p.recs.forEach(function(e){var n=p.refMap[e.bean_id];n&&t.forEach(function(t){var o=i(e[t.col]),r=i(n[t.col]);null!=o&&null!=r&&u.push(Math.abs(o-r))})});var w,h=u.length?u.reduce(function(e,t){return e+t},0)/u.length:null,g=null==h?"#b0b8c1":h<=1?"#00b386":h<=2.5?"#e08600":"#e5484d",x=null==f,m=(w=[],p.recs.forEach(function(e){t.forEach(function(t){var n=i(e[t.col]);null!=n&&w.push(n)})}),w.length?w.reduce(function(e,t){return e+t},0)/w.length:null),v='<div class="wc-ch-card"><div style="font-size:12px;color:#8b95a1;font-weight:600;margin-bottom:4px;">'+n(l)+'</div><div style="font-size:17px;font-weight:700;color:#191f28;line-height:1.35;word-break:keep-all;margin-bottom:14px;">'+n(c)+'</div><div class="wc-ch-scorerow"><div class="wc-ch-sc"><div class="l">'+(x?"내 평균 강도":"내 평균 점수")+'</div><div class="v" style="color:#ea6f00;">'+o(x?m:f)+'</div></div><div class="wc-ch-sc"><div class="l">레퍼런스 평균 편차</div><div class="v" style="color:'+g+';">'+(null==h?"—":h.toFixed(1))+"</div></div></div>"+(null!=h?'<div style="font-size:11.5px;color:#8b95a1;margin-top:8px;text-align:center;">편차가 작을수록 레퍼런스에 가깝게 평가한 거예요.</div>':"")+'</div><div class="wc-ch-tabs" id="wcCupTabs"></div><div id="wcCupToggleWrap"></div><div id="wcCupView"></div>';e.innerHTML=v,z()}(s),T()}catch(e){console.error("[cupping] 커핑 내역 조회 실패",e),S(s,c,"내역을 불러오지 못했어요.","잠시 후 다시 시도하거나, 관리자에게 문의해 주세요.")}finally{window.hid&&window.hid("ld")}}else window.cAl&&window.cAl("안내","먼저 로그인 해주세요.")},window.wcCupTab=function(e){p.activeBean=e;var t=E(e);t&&"basic"===t.form_type&&"form"===p.view&&(p.view="card"),z(),T()},window.wcCupToggleView=function(e){p.view=e,T()},window.wcEnhanceCupHistCards=M,H("renderTrainings"),H("loadMoreTrn"),C(),setTimeout(M,300),setTimeout(M,1200),"loading"===document.readyState&&document.addEventListener("DOMContentLoaded",function(){setTimeout(M,500)})}();
!function(){"use strict";function e(e){return String(null==e?"":e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function t(){return document.querySelector("#wcCupHistBody .wc-ch-radar")}function r(r){var n=t();if(n){var c=n.querySelector(".wc-ch-legend");if(c){var o=null;if(c.querySelectorAll("span").forEach(function(e){var t=e.querySelector("i");t&&/12b886|18,\s*184,\s*134/.test(t.getAttribute("style")||"")&&(o=e)}),o)o.innerHTML='<i style="background:#12b886"></i>'+e(r||"참가자 평균");else if(r){var a=document.createElement("span");a.innerHTML='<i style="background:#12b886"></i>'+e(r),c.appendChild(a)}}}}document.addEventListener("click",function(e){var n=e.target.closest?e.target.closest("#wcCupHistBody .wc-ch-pcard[data-vals]"):null;if(n&&"function"==typeof window._wcChRadarFn){var c=t();if(c){var o=c.querySelector("svg");if(o){var a=(n.getAttribute("data-vals")||"").split(",").map(function(e){return""===e?null:Number(e)}),i="1"===n.getAttribute("data-sel");document.querySelectorAll("#wcCupHistBody .wc-ch-pcard").forEach(function(e){e.removeAttribute("data-sel"),e.style.borderColor="#f0f1f3",e.style.background="#fff",e.style.boxShadow="none"});try{i?(o.outerHTML=window._wcChRadarFn(window._wcChMe,window._wcChRef,window._wcChAvg,window._wcChAx),r(null)):(n.setAttribute("data-sel","1"),n.style.borderColor="#12b886",n.style.background="#eefaf4",n.style.boxShadow="0 0 0 2px rgba(18,184,134,.28)",o.outerHTML=window._wcChRadarFn(window._wcChMe,window._wcChRef,a,window._wcChAx),r(n.getAttribute("data-nm")))}catch(e){console.error("[cupping] 마이페이지 탭 비교 오류",e)}}}}},!0)}();
