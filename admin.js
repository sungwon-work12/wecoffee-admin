...
if (!document.getElementById('wecoffee-custom-styles')) {
    ...
    style.innerHTML = `
        ...
        @media (max-width: 768px) {
            /* 💡 3. 멤버리스트 UI 폼 모바일 이탈 방지용 wrap 교정 */
...
// 💡 4. 생두 날짜/요일 정확한 파싱 및 포맷 (대괄호 제거형)
...
// 💡 1. [필터 먹통 해결] 글로벌 센터 필터 탭 클릭 시 즉각 반응
window.changeGlobalCenter = function(centerName) {
    currentGlobalCenter = centerName; // 변수 최신화
    ...
    window.fetchCenterData(); // 화면 즉시 다시 그리기 트리거
};
...
// 💡 4. [생두 UI/로직 완전체] 여백 밸런스, 날짜 위치, 뱃지 통일
...
// 💡 5. [고질적 버그 완치] 내역 삭제 시 월 빼기 역산 복구 안전장치
...
// 💡 4. [개선 4] 선택된 항목만 뽑아 요약하는 스마트 요약 모달 구현 및 한글 매핑
