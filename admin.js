...
window.fetchCenterData = async function() {
  try {
    ...
    try { updateSmartBadges(); } catch(e){ console.error(e); }
...
// 🔥 어드민 드롭다운 및 뱃지 상태에 "입금 확인 중" 완벽 추가!
    ...
    scheduledApps.forEach(app => { 
        ...
        if (m) { 
            ...
            if (calEvts[ds]) { 
                ...
                let timeStr = tm ? `${tm[1]} ${tm[2]}:${tm[3]}` : (app.call_time.includes(')') ? app.call_time.split(')')[1].trim() : app.call_time); 
...
window.renderMCalApp = function(selDate) { 
    ...
    if(evts.length === 0) { 
        ...
        evts.forEach(e => { 
            html += `<div class="m-cal-card"><div class="m-cal-card-time" style="color:var(--primary);">${e.time || '종일'}</div><div class="m-cal-card-title">${window.escapeHtml(e.text)||''}</div><div class="m-cal-card-desc">${String(e.tooltip||'').split('|').slice(1).join('|').trim()}</div></div>`; 
        }); 
    } 
...
window.renderAppDailyBanner = function(data) {
  ...
  if(todayEvts.length === 0) { 
      ...
      todayEvts.forEach(evt => { 
          ...
          html += `<div class="inout-card" style="padding: 16px; gap: 8px; border-radius:12px; border:1px solid var(--border-strong); background:#fff;">
...
window.openScheduleModal = function(id, time, name) { 
    ...
    $("schedInputDate").value = ""; $("schedInputDate").dataset.raw = "";
    $("schedInputTime").value = ""; $("schedInputTime").dataset.raw = "";
