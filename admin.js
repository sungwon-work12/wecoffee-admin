...
// 💡 1. 툴팁 UI, N회차 뱃지, 캘린더 짤림 방지 및 멤버리스트 1줄 정렬용 CSS
if (!document.getElementById('wecoffee-custom-styles')) {
    ...
    style.innerHTML = `
        ...
        .date-inputs select { flex-shrink: 0; width: auto !important; min-width: 70px; }
    `;
...
// 💡 [이슈 해결] Safari, iOS 등 모든 환경에서 undefined를 내뱉지 않는 철벽 날짜 파서
function safeDate(dateStr) {
    if(!dateStr) return new Date();
    let d = new Date(dateStr);
    if(isNaN(d.getTime())) d = new Date(String(dateStr).replace(/-/g, '/').replace('T', ' ').split('.')[0]);
...
// 💡 렌더링 시 전부 safeDate 적용 (undefined 원천 차단)
function getDow(dStr) { if(!dStr) return ''; const d = safeDate(dStr); return ['일','월','화','수','목','금','토'][d.getDay()]; }
function formatDtWithDow(dateStr) { if(!dateStr) return "-"; const d = safeDate(dateStr); const dow = ['일','월','화','수','목','금','토'][d.getDay()]; return `${d.getFullYear().toString().slice(-2)}/${String(d.getMonth()+1).padStart(2,'0')}/${String(d.getDate()).padStart(2,'0')}(${dow}) ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`; }
function formatDt(dateStr) { if(!dateStr) return "-"; const d = safeDate(dateStr); return `${d.getFullYear().toString().slice(-2)}/${String(d.getMonth()+1).padStart(2,'0')}/${String(d.getDate()).padStart(2,'0')} ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`; }
...
window.toggleAll = function(checkbox, targetClass) { const checkboxes = document.querySelectorAll('.' + targetClass); checkboxes.forEach(cb => { if (!cb.disabled) cb.checked = checkbox.checked; }); };
...
// 💡 [이슈 해결] 공지사항 저장 시 에러 팝업 고도화 및 DB 반영 처리
...
window.openBlockModal = function() { 
    currentBlockId = null; if($("blockModalTitle")) $("blockModalTitle").innerText = "수업 및 훈련 등 등록"; if($("blkId")) $("blkId").value = ""; if($("blkCategory")) $("blkCategory").value = "기본 수업"; if($("blkDate")) $("blkDate").value = ""; if($("blkStart")) $("blkStart").value = ""; if($("blkEnd")) $("blkEnd").value = ""; 
    if($("blkCenter")) { $("blkCenter").value = currentGlobalCenter === '전체' ? '마포 센터' : currentGlobalCenter; $("blkCenter").onchange = window.updateSpaceOptions; window.updateSpaceOptions(); }
    if($("blkSpace")) $("blkSpace").value = ""; if($("blkReason")) $("blkReason").value = ""; if($("blkCapacity")) $("blkCapacity").value = ""; if($("blkRepeatType")) $("blkRepeatType").value = "none"; if($("blkRepeatCount")) $("blkRepeatCount").value = ""; if($("blockModal")) $("blockModal").classList.add('show'); 
}
window.editBlock = function(id) { 
    ...
    if($("blkSpace")) $("blkSpace").value = b.space_equip; if($("blkReason")) $("blkReason").value = b.reason; 
    if($("blkCapacity")) $("blkCapacity").value = b.capacity === null ? "" : b.capacity; 
    if($("blkRepeatType")) $("blkRepeatType").value = "none"; if($("blkRepeatCount")) $("blkRepeatCount").value = ""; if($("blockModal")) $("blockModal").classList.add('show'); 
}
...
window.saveBlockData = async function() { 
  ...
  const payload = { 
      ...
      start_time: window.formatBlockTime($("blkStart")?$("blkStart").value:""), end_time: window.formatBlockTime($("blkEnd")?$("blkEnd").value:""), 
      center: $("blkCenter")?$("blkCenter").value:"", space_equip: $("blkSpace")?$("blkSpace").value.trim():"", 
      reason: $("blkReason")?$("blkReason").value.trim():"", capacity: capParsed 
  }; 
  ...
  for(let i=0; i<rCount; i++) { let currentPayload = { ...payload }; let targetDate = new Date(baseDate); if(rType === 'weekly') { targetDate.setDate(targetDate.getDate() + (i * 7)); } else if(rType === 'monthly') { targetDate.setMonth(targetDate.getMonth() + i); } currentPayload.block_date = `${targetDate.getFullYear()}-${String(targetDate.getMonth()+1).padStart(2,'0')}-${String(targetDate.getDate()).padStart(2,'0')}`; payloads.push(currentPayload); }
  let error; if(currentBlockId) { const res = await supabaseClient.from('blocks').update(payload).eq('id', currentBlockId); error = res.error; } else { const res = await supabaseClient.from('blocks').insert(payloads); error = res.error; } 
  if(error) showToast("저장 실패"); else { showToast(`성공적으로 저장되었습니다.`); window.closeBlockModal(); window.fetchCenterData(); } 
}
...
window.applyGlobalCenter = function() { let r = document.querySelector('input[name="globalCenter"]:checked'); currentGlobalCenter = r ? r.value : '전체'; if(window.updateDashSpaceFilter) window.updateDashSpaceFilter(); window.renderCenterData(); window.renderDashboard(); }
...
window.updateCancelAccumulationBanner = function() {
    ...
    gTrn.forEach(t => { if (t.status === '당일 취소') { let cInfo = String(t.content||'').split(' || '); let dateStr = cInfo.length >= 5 ? cInfo[0].trim() : String(t.created_at); if(dateStr.startsWith(monthPrefix)) addCancel(t.phone, t.name, t.batch); } });
...
window.renderCenterData = function() {
  ...
  try {
      ...
      let fTrnList = gTrn.filter(t => { 
          let matchContent = true; let cInfo = String(t.content||'').split(' || ');
          if(sTrn !== '전체') { let targetStr = cInfo.length >= 5 ? `[${cInfo[0].trim()}] [${cInfo[2].trim()}] ${cInfo[4].trim()}` : String(t.content||''); if(targetStr !== sTrn) matchContent = false; }
          if (cInfo.length >= 5) { let tDateObj = new Date(cInfo[0].trim()); tDateObj.setHours(0,0,0,0); if (tDateObj < todayForBlk) return false; } else { let tDate = safeKST(t.created_at); if (tDate < oneMonthAgo) return false; }
  ...
  // 💡 [이슈 해결] 생두 주문 관리: 요일 에러(undefined) 해결 및 [날짜 기반 다이내믹 그룹핑] 완벽 적용 (이모지 제외)
  try {
      ...
      let fOrd = gOrd.filter(o => { 
        let matchCenter = (currentGlobalCenter === '전체' || o.center === currentGlobalCenter); let matchQ = `${o.name} ${o.phone} ${o.vendor} ${o.item_name} ${o.center||''}`.toLowerCase().includes(qOrd); 
        let matchV = vOrd === '전체' ? true : o.vendor === vOrd; let matchS = isOrdFilter ? (o.status==='주문 접수'||o.status==='입금 대기'||o.status==='입금 확인 중'||o.status==='입금 확인') : true; 
        return matchCenter && matchQ && matchV && matchS; 
      }); 
      
      if (!isOrdFilter) { fOrd = fOrd.filter(o => !isOrderExpired(o, now)); }
      
      let orderGroups = {};
      fOrd.forEach(o => {
          let dObj = safeKST(o.created_at);
          let dateKey = `${dObj.getFullYear()}-${String(dObj.getMonth()+1).padStart(2,'0')}-${String(dObj.getDate()).padStart(2,'0')}`;
          let dateLabel = `${dObj.getMonth()+1}월 ${dObj.getDate()}일 ${getDow(o.created_at)}요일 발주건`;
          if(!orderGroups[dateKey]) orderGroups[dateKey] = { label: dateLabel, items: [] };
          orderGroups[dateKey].items.push(o);
      });
      
      let sortedDateKeys = Object.keys(orderGroups).sort((a,b) => new Date(b) - new Date(a));
      
      let finalOrdHtml = '';
      if(sortedDateKeys.length === 0) {
          finalOrdHtml = `<tr><td colspan="10" class="empty-state">조건에 일치하는 주문 내역이 없습니다.</td></tr>`;
      } else {
          sortedDateKeys.forEach(dk => {
              let group = orderGroups[dk];
              finalOrdHtml += `<tr style="background:#f4f5f7;"><td colspan="10" style="font-weight:800; font-size:15px; color:var(--text-display); text-align:left; padding:16px; border-top:1px solid var(--border-strong); border-bottom:1px solid var(--border-strong);">[ ${group.label} ]</td></tr>`;
              finalOrdHtml += generateOrderRows(group.items, 'chk-ord');
          });
      }
      
      if($("ordTableBodyMon")) $("ordTableBodyMon").innerHTML = finalOrdHtml;
      
      // 기존 하드코딩된 두 번째 테이블 영역은 완전히 숨김 처리하여 다이내믹 통합 렌더링 체제 적용
      let thuContainer = $("ordTableBodyThu")?.closest('.table-wrap');
      if(thuContainer) thuContainer.style.display = 'none';
      let thuTitle = document.querySelector('#sub-ord .section-title:nth-of-type(2)');
      if(thuTitle) thuTitle.style.display = 'none';
    ...
    try {
        ...
        iterDates.forEach(ds => {
            ...
            let evtsHtml = evts.slice(0, 3).map(e => {
                ...
                return `<div class="dash-item ${badgeClass}"><div class="dash-item-text"><span class="dash-time">${e.time||''}</span>${window.escapeHtml(e.text)||''}</div><div class="dash-tooltip-custom">${window.escapeHtml(e.tooltip)||''}</div></div>`;
            }).join('');

            if(evts.length > 3) {
                let hiddenText = evts.slice(3).map(e => `<div style="padding:4px 0; line-height:1.4;">${e.time||''} | ${window.escapeHtml(e.text)||''}</div>`).join('');
                evtsHtml += `<div class="dash-cal-more-wrap" style="position:relative;"><div class="dash-cal-more">+${evts.length - 3}건 더보기</div><div class="dash-tooltip-custom">${hiddenText}</div></div>`;
            }
...
window.renderAppDailyBanner = function(filteredApps) {
    let today = new Date(); let y = today.getFullYear(); let m = String(today.getMonth() + 1).padStart(2, '0'); let d = String(today.getDate()).padStart(2, '0');
    let targetDateRegex = new RegExp(`${y}년\\s*0?${parseInt(m)}월\\s*0?${parseInt(d)}일`);
    let todaysApps = filteredApps.filter(app => { if(app.status !== '상담 일정 확정') return false; let cTime = String(app.call_time||''); return targetDateRegex.test(cTime); });
    todaysApps.sort((a,b) => { let tA = String(a.call_time||'').match(/(오전|오후)\s+(\d+):(\d+)/); let tB = String(b.call_time||'').match(/(오전|오후)\s+(\d+):(\d+)/); let timeA = tA ? (tA[1]==='오후' && tA[2]!=='12' ? parseInt(tA[2])+12 : (tA[1]==='오전' && tA[2]==='12' ? 0 : parseInt(tA[2]))) * 60 + parseInt(tA[3]) : 0; let timeB = tB ? (tB[1]==='오후' && tB[2]!=='12' ? parseInt(tB[2])+12 : (tB[1]==='오전' && tB[2]==='12' ? 0 : parseInt(tB[2]))) * 60 + parseInt(tB[3]) : 0; return timeA - timeB; });
    let html = '';
    if(todaysApps.length === 0) { html = `<div class="inout-card" style="text-align:center; color:var(--text-secondary); width:100%; padding:16px; border-radius:12px; border:1px solid var(--border-strong); background:#fff;">오늘 예정된 상담이 없습니다.</div>`; } else { todaysApps.forEach(app => { let tm = String(app.call_time||'').match(/(오전|오후)\s+(\d+):(\d+)/); let tStr = tm ? `${tm[1]} ${tm[2]}:${tm[3]}` : '시간 미정'; html += `<div style="padding:12px 16px; border:1px solid var(--border-strong); background:#fff; border-radius:12px; flex: 1; min-width:240px; box-sizing:border-box; display: flex; flex-direction: column; justify-content: center;"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;"><div style="font-weight:800; color:var(--text-display); font-size:15px;">[${app.desired_batch||'-'}] ${window.escapeHtml(app.name)}</div><div style="font-weight:800; color:var(--primary); font-size: 13px;">${tStr}</div></div><div style="font-size:13px; color:var(--text-secondary);">담당자: <span style="font-weight:600;">${window.escapeHtml(app.counselor_name||'미정')}</span></div></div>`; }); html = `<div style="display:flex; flex-wrap:wrap; gap:12px;">${html}</div>`; }
...
window.searchMembers = function() { 
    ...
    filtered.sort((a, b) => {
        ...
        let batchB = b.batch || '';
        if (batchA !== batchB) {
            return parseInt(batchB.replace(/[^0-9]/g, '') || 0) - parseInt(batchA.replace(/[^0-9]/g, '') || 0);
        }
        let nameA = a.name || '';
        let nameB = b.name || '';
...
function renderMemberTablePage() {
  ...
  let getCancelCount = (phone) => {
      let count = 0;
      gRes.forEach(r => { if(r.phone === phone && r.status === '당일 취소' && String(r.res_date || r.created_at).startsWith(monthPrefix)) count++; });
      gTrn.forEach(t => { 
          if(t.phone === phone && t.status === '당일 취소') {
              let dStr = String(t.content||'').split(' || ')[0] || String(t.created_at);
              if(dStr.startsWith(monthPrefix)) count++;
          } 
      });
      return count;
  };
  ...
  pageData.forEach(row => {
    ...
    // 💡 [이슈 해결] 멤버 리스트 날짜 관리부 UI 1줄(flex-wrap: nowrap)로 완벽 고정
    ...
    const tr = document.createElement('tr'); tr.innerHTML = `${mPreview}<td data-label="등록일">${formatDt(row.created_at)}</td><td data-label="상태" class="tc">${statusBadge}</td><td data-label="기수"><strong>${row.batch || '-'}</strong></td><td data-label="성함">${nameHtml}</td><td data-label="연락처">${window.escapeHtml(row.phone) || '-'}</td><td data-label="종료일 관리" class="col-action" style="white-space:nowrap;"><div class="date-select-group" data-id="${row.id}" style="display:flex; align-items:center; gap:8px;"><div class="date-inputs" style="display:flex; align-items:center; gap:4px; flex-wrap:nowrap;"><select class="date-sel year" style="width:auto; padding-right:20px;">${yearOpts}</select><select class="date-sel month" style="width:auto; padding-right:20px;">${monthOpts}</select><select class="date-sel day" style="width:auto; padding-right:20px;">${dayOpts}</select>${dateActionHtml}</div><div class="action-btns" style="display:flex; align-items:center; gap:4px; flex-wrap:nowrap;"><select class="date-sel option-btn" style="width:120px; padding-right:20px;" onchange="window.handleMemberOption('${row.id}', '${row.batch || '미정'}', '${window.escapeHtml(row.name)}', '${window.escapeHtml(row.phone)}', '${row.end_date || ''}', this)"><option value="">옵션 선택</option><option value="1">1개월 연장</option><option value="3">3개월 연장</option><option value="6">6개월 연장</option><option value="bonus">보너스 1개월</option><option value="day">당일권 추가</option><option value="pause">활동 일시정지</option><option value="resume">활동 재개 (자동 연장)</option><option value="release">패널티 적용/해제</option></select><button class="btn-outline btn-sm" style="flex-shrink:0; height:32px;" onclick="event.stopPropagation(); window.openHistoryModal('${window.escapeHtml(row.phone)}', '${window.escapeHtml(row.name)}')">내역</button></div></div></td>`; tbody.appendChild(tr);
  });
...
window.applyMemberDate = async function(id, btn) {
    ...
    if (y && m && d) {
        ...
        window.fetchMembers();
    } else { showToast("년/월/일을 모두 선택해주세요."); }
};
...
window.downloadAttendanceExcel = function() {
    if(!window.currentFilteredTrn || window.currentFilteredTrn.length === 0) {
        showToast('출력할 데이터가 없습니다.');
        return;
    }
    let csv = "\uFEFF기수,성함,연락처,참여 회차,상태,수업 정보\n";
    window.currentFilteredTrn.forEach(t => {
        let cInfo = String(t.content||'').split(' || ');
        let classInfo = cInfo.length >= 5 ? `[${cInfo[0]}] ${cInfo[2]} ${cInfo[4]}` : t.content;
        csv += `"${t.batch||'-'}","${String(t.name).replace(/"/g, '""')}","${String(t.phone).replace(/"/g, '""')}","${t._attendCount || 1}회차","${t.status}","${String(classInfo).replace(/"/g, '""')}"\n`;
    });
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' }); 
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); 
    link.download = `위커피_참가자리스트_${new Date().toISOString().slice(0,10)}.csv`; link.click(); 
};
...
// 💡 [이슈 해결] 발주 요약 모달 - 이모지 제거, 신규 건 필터링, g/kg 완벽 합산 로직
window.showOrderSummary = function() {
    ...
    if (pendingOrders.length === 0) {
        ...
        pendingOrders.forEach(o => {
            ...
            let dObj = safeDate(o.created_at);
        ...
        let exportData = []; 
        sortedData.forEach(s => { 
            s.orderers.forEach(o => { exportData.push({ dateGroup: s.dateGroup, center: s.center, vendor: s.vendor, item: s.item, rawQty: o.rawQty, batch: o.batch, name: o.name, phone: o.phone }); }); 
        });
