...
window.renderCrmInner=function(id,isReadOnly=false){const app=globalApps.find(a=>String(a.id)===String(id));if(!app)return;let cCount=0;let now=new Date();let monthPrefix=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;if(gRes&&gTrn){gRes.forEach(r=>{if(window.samePhone(r.phone,app.phone)&&r.status==='당일 취소'&&String(r.res_date||r.created_at).startsWith(monthPrefix))cCount++;});gTrn.forEach(t=>{if(window.samePhone(t.phone,app.phone)&&t.status==='당일 취소'){let dStr=String(t.content||'').split(' || ')[0]||String(t.created_at);if(dStr.startsWith(monthPrefix))cCount++;}});}let warnHtml=cCount>=4?`<span style="background:var(--error);color:#fff;font-size:11px;padding:2px 6px;border-radius:4px;margin-left:8px;font-weight:700;vertical-align:middle;">경고</span>`:'';if($("crmName"))$("crmName").innerHTML=`${window.escapeHtml(app.name||'이름 없음')} ${warnHtml}`;// ★ 요청1: 프로필을 라벨+값 쌍 구조로 재구성 — 각 쌍은 nowrap으로 줄바꿈 방지
    let _pair = (label, value, valueStyle) => `<span style="white-space:nowrap;"><span style="color:var(--text-tertiary);font-size:12px;font-weight:600;">${label}</span>&nbsp;<span style="${valueStyle||'font-weight:700;color:var(--text-display);'}">${value}</span></span>`;
    let _profileItems = [];
    _profileItems.push(`<span style="font-weight:800;color:var(--text-display);font-size:15px;white-space:nowrap;">[${app.desired_batch||'미정'}]</span>`);
    _profileItems.push(_pair('연락처', window.escapeHtml(window.normalizePhone(app.phone)||app.phone||'-')));
    if(app.acquisition_channel) _profileItems.push(_pair('유입', window.escapeHtml(app.acquisition_channel)));
    if(app.desired_center) _profileItems.push(_pair('희망센터', window.escapeHtml(app.desired_center)));
    if(app.interest_level) _profileItems.push(_pair('관심도', window.escapeHtml(app.interest_level), 'font-weight:700;color:var(--primary);'));
    let _divider = `<span style="color:var(--border-strong);margin:0 8px;flex-shrink:0;">|</span>`;
    if($("crmProfile"))$("crmProfile").innerHTML=`<div style="display:flex;flex-wrap:wrap;align-items:center;gap:4px 0;line-height:1.8;">${_profileItems.join(_divider)}</div>`;
...
window.renderAppTable = function(data) {
    ...
    data.forEach(row=>{
        ...
        // ★ NEW: interest_level을 모바일 카드에 표시 (주황색, m-prev-title 바로 아래)
        ...
        // ★ 요청2: 모바일 카드에 유입경로, 희망센터, 통화선호시간/상담예정일 추가 — nowrap으로 라벨+값 쌍 보호
        let _mRoute = row.acquisition_channel ? `<span style="white-space:nowrap;"><span style="color:var(--text-tertiary);font-size:11px;font-weight:600;">유입</span>&nbsp;<span style="font-weight:600;color:var(--text-secondary);">${window.escapeHtml(row.acquisition_channel)}</span></span>` : '';
        let _mCenter = row.desired_center ? `<span style="white-space:nowrap;"><span style="color:var(--text-tertiary);font-size:11px;font-weight:600;">희망센터</span>&nbsp;<span style="font-weight:600;color:var(--text-secondary);">${window.escapeHtml(row.desired_center)}</span></span>` : '';
        ...
        let _mTimeHtml = _mRawTime ? `<div style="font-size:12px;margin-top:4px;white-space:nowrap;"><span style="color:var(--text-secondary);font-weight:700;">${_mTimeLabel}</span>&nbsp;<span style="color:var(--text-display);font-weight:600;">${window.escapeHtml(_mRawTime)}</span></div>` : '';
        let mPreview=`<td class="m-preview" onclick="this.closest('tr').classList.toggle('expanded')"><div class="m-prev-top"><span class="m-prev-date">${formatDtWithDow(row.created_at)}</span><span class="status-badge ${cStat}">${row.status}</span></div><div class="m-prev-title">[${row.desired_batch||'-'}] ${window.escapeHtml(row.name)} <span style="font-size:13px;color:var(--text-secondary);">(${window.escapeHtml(window.normalizePhone(row.phone)||row.phone||'')})</span></div>${levelHtml}<div class="m-prev-desc">${window.escapeHtml(row.interest_area)}</div>${_mMetaHtml}${_mTimeHtml}</td>`;
...
window.currentInsightData={total,joined,dropoutCount,realDropoutRate,applyDropoutRate,instaCount:channelMap['인스타그램']?channelMap['인스타그램'].total:0,adCount:channelMap['모집 광고']?channelMap['모집 광고'].total:0,instaFollow:safeDataForSummary.instaFollow,instaNonFollow:safeDataForSummary.instaNonFollow,leadTime1M:safeDataForSummary.adNow,leadTime3M:safeDataForSummary.leadTime3M,channelMap,counselorMap};}
...
window.downloadExcel = function(type) {
    try {
        ...
        if(type === 'applications') {
            ...
            filename = "가입신청";
            headers = ['신청일','기수','성함','연락처','관심분야','관심도','유입경로','진행상황','가입여부','상담일시','담당자'];
        }
        ...
        data.forEach(d => {
            ...
            if(type === 'applications')      row = [formatDt(d.created_at), d.desired_batch, d.name, phoneOut, d.interest_area, d.interest_level, d.acquisition_channel, d.status, d.join_status, d.call_time, d.counselor_name];
