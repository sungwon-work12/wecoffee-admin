...
let isLandscapeMode = false; // 가로 모드 상태 변수 추가
...
wecoffeeStyle.innerHTML = `
    ...
    /* 타임라인 강제 레이아웃 보장 및 좌측 고정(Sticky) CSS */
    #timeline-area { width: 100%; max-width: 100vw; box-sizing: border-box; margin-top: 32px; display: block !important; clear: both; transition: all 0.3s ease; }
    #timeline-area .timeline-section { width: 100%; margin: 0 0 32px 0 !important; background: #fff; padding: 24px; border-radius: 12px; border: 1px solid var(--border-strong); box-shadow: 0 4px 20px rgba(0,0,0,0.05); box-sizing: border-box; overflow: hidden; text-align: left; display: block !important; position: relative; }
    
    /* 모바일 가로 모드 (전체 화면) 지원 CSS */
    body.is-landscape #timeline-area {
        position: fixed;
        top: 0;
        left: 0;
        width: 100vw;
    ...
    body.is-landscape #timeline-area .timeline-section {
        height: 100%;
    ...
    body.is-landscape .timeline-container {
        flex: 1;
        height: 100%;
        overflow-y: auto;
    }
    body.is-landscape .fullscreen-btn svg {
        transform: rotate(180deg); /* 아이콘 반전으로 축소 의미 부여 */
    }
    ...
    #rotate-overlay svg { animation: rotatePhone 2s infinite ease-in-out; margin-bottom: 16px; }
...
// 전체화면 토글 함수
window.toggleFullscreenTimeline = function() {
...
function showRotateOverlay() {
    ...
    if (!overlay) {
        ...
        overlay.innerHTML = `
            ...
            <div style="font-size: 18px; font-weight: 700; margin-bottom: 8px;">기기를 가로로 회전해주세요</div>
            <div style="font-size: 14px; color: #ccc; margin-bottom: 24px;">더 넓은 화면으로 타임라인을 볼 수 있습니다.</div>
            <button onclick="window.hideRotateOverlay()" style="background: var(--primary); color: white; border: none; padding: 10px 24px; border-radius: 8px; font-weight: 600; cursor: pointer;">닫기</button>
        `;
    ...
    // 화면 회전 감지하여 자동으로 오버레이 숨김
    window.addEventListener('resize', handleOrientationChange);
}

window.hideRotateOverlay = function() {
    let overlay = document.getElementById('rotate-overlay');
...
                    // 💡 수정: 불필요한 꼬리 콤마 강제 삽입 제거 (배열 결합만 수행)
...
// 💡 누락되었던 메인 대시보드(센터 일정) 전환 및 월 이동 함수 복구
...
    // 💡 HTML 렌더링에 merged-col 클래스 추가 반영 및 전체화면 버튼 추가 (가로모드)
    let finalHtml = `
...
                </div>
                <button class="fullscreen-btn" onclick="window.toggleFullscreenTimeline()" title="전체 화면 (가로 모드)">
