...
function generateOrderRows(fOrd, chkClass, dateKey) {
  return fOrd.map(o => {
    ...
    let paySelect = `<select class="status-select ${payClass}" onchange="window.handlePaymentStatus('${o.id}',this.value,this)" style="text-align-last:center;font-size:12px;padding:4px 2px;" ${payDisabled}><option value="주문 접수" ${payStatus==='주문 접수'?'selected':''}>주문 접수</option><option value="입금 대기" ${payStatus==='입금 대기'?'selected':''}>입금 대기</option><option value="입금 확인 중" ${payStatus==='입금 확인 중'?'selected':''}>입금 확인 중</option><option value="입금 확인" ${payStatus==='입금 확인'?'selected':''}>입금 확인</option><option value="주문 취소" ${payStatus==='주문 취소'?'selected':''}>주문 취소</option><option value="품절" ${payStatus==='품절'?'selected':''}>품절</option></select>`;
    let delSelect = `<select class="status-select ${delClass}" onchange="window.handleDeliveryStatus('${o.id}',this.value,this)" style="text-align-last:center;font-size:12px;padding:4px 2px;" ${delEnabled?'':'disabled'}><option value="" ${!isArrived?'selected':''}>—</option><option value="센터 도착" ${isArrived?'selected':''}>센터 도착</option></select>`;
...
window.toggleOrderDetail=function(orderId,dateKey){
    ...
    let itemsHtml=samePersonOrders.map((o,i)=>{
        ...
        let isThis=String(o.id)===String(orderId);
        return `<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid var(--border-strong);font-size:12px;${isThis?'background:#FFF6EF;margin:0 -4px;padding-left:4px;padding-right:4px;border-radius:4px;':''}"><span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--text-display);">${window.escapeHtml(o.vendor||'')} | ${window.escapeHtml(nm)}</span><span style="flex-shrink:0;width:40px;text-align:center;color:var(--text-secondary);">${o.quantity}</span><span style="flex-shrink:0;width:70px;text-align:right;font-weight:600;">${o.total_price||'-'}</span></div>`;
    }).join('');
    ...
    let dow=getDow(order.created_at);
    let tr=document.createElement('tr');tr.id='ord-detail-'+orderId;tr.style.cssText='border-bottom:1px solid var(--border-strong);';
    tr.innerHTML=`<td colspan="8" style="padding:0;background:var(--color-background-secondary,#f9fafb);"><div style="padding:14px 16px;display:flex;gap:20px;"><div style="min-width:130px;display:flex;flex-direction:column;gap:10px;"><div><div style="font-size:11px;color:var(--text-tertiary);font-weight:600;margin-bottom:2px;">주문 시간</div><div style="font-size:13px;font-weight:600;">${formatDtWithDow(order.created_at)}</div></div><div><div style="font-size:11px;color:var(--text-tertiary);font-weight:600;margin-bottom:2px;">수령 센터</div><div style="font-size:13px;font-weight:600;">${order.center||'미지정'}</div></div><div><div style="font-size:11px;color:var(--text-tertiary);font-weight:600;margin-bottom:2px;">멤버 연락처</div><div style="font-size:13px;font-weight:600;">${window.escapeHtml(window.normalizePhone(order.phone)||order.phone||'-')}</div></div></div><div style="flex:1;min-width:0;"><div style="font-size:12px;font-weight:600;color:var(--text-secondary);margin-bottom:8px;">${window.escapeHtml(order.name)} 님의 발주 명세서</div>${itemsHtml}<div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px;padding-top:10px;border-top:2px solid var(--border-strong);"><span style="font-size:13px;font-weight:600;color:var(--text-secondary);">총 청구 금액 (${samePersonOrders.length}건)</span><span style="font-size:18px;font-weight:800;color:var(--primary);">${totalStr}</span></div><div style="margin-top:10px;"><button class="btn-outline btn-sm" style="font-size:12px;padding:6px 12px;" onclick="window.copyOrderInvoice('${orderId}','${dateKey.replace(/'/g,"\\'")}')">명세서 복사하기</button></div></div></div><div style="font-size:11px;color:var(--text-tertiary);text-align:right;padding:4px 16px 8px;cursor:pointer;" onclick="document.getElementById('ord-detail-${orderId}').style.display='none'">접기 ▲</div></td>`;
...
window.copyOrderInvoice=function(orderId,dateKey){
    let order=gOrd.find(o=>String(o.id)===String(orderId));if(!order)return;
    let samePersonOrders=gOrd.filter(o=>window.samePhone(o.phone,order.phone)&&window.formatDeliveryDateFull(o.delivery_date)===dateKey);
    let lines=[`[위커피] ${order.name} 님 ${dateKey} 명세서`,``];
    let totalAmt=0;
    samePersonOrders.forEach((o,i)=>{
        let nm=o.item_name||'';let m=String(nm).match(/(.+) \[(?:희망:\s*)?(\d+)[\/\.](\d+).*?\]/);if(m)nm=m[1].trim();else{let oM=String(nm).match(/(.+) \[(.*?)\]/);if(oM)nm=oM[1].trim();}
        let price=o.total_price||'미입력';lines.push(`${i+1}. ${nm} (${o.quantity}) — ${price}`);
...
// ★★★ PATCH: applyFilterApp이 만든 필터링 결과를 캐싱 → downloadExcel('applications')에서 재사용 ★★★
...
window.renderCrmInner=function(id,isReadOnly=false){const app=globalApps.find(a=>String(a.id)===String(id));if(!app)return;let cCount=0;let now=new Date();let monthPrefix=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;if(gRes&&gTrn){gRes.forEach(r=>{if(window.samePhone(r.phone,app.phone)&&r.status==='당일 취소'&&String(r.res_date||r.created_at).startsWith(monthPrefix))cCount++;});gTrn.forEach(t=>{if(window.samePhone(t.phone,app.phone)&&t.status==='당일 취소'){let dStr=String(t.content||'').split(' || ')[0]||String(t.created_at);if(dStr.startsWith(monthPrefix))cCount++;}});}let warnHtml=cCount>=4?`<span style="background:var(--error);color:#fff;font-size:11px;padding:2px 6px;border-radius:4px;margin-left:8px;font-weight:700;vertical-align:middle;">경고</span>`:'';if($("crmName"))$("crmName").innerHTML=`<span style="font-weight:800;color:var(--text-display);margin-right:4px;">[${app.desired_batch||'미정'}]</span> ${window.escapeHtml(app.name||'이름 없음')} ${warnHtml}`;// ★ 요청1: 프로필을 라벨 폭 고정 그리드로 재구성
    ...
    if(app.interest_level) _profileHtml += _row('관심도', window.escapeHtml(app.interest_level), 'font-weight:700;color:var(--primary);font-size:14px;');
    ...
    // ★ 요청3: call_time 라벨 분기 + 프로필 블록과 시각적 분리
...
window.renderAppTable = function(data) {
    ...
    data.forEach(row=>{
        ...
        // ★ m-preview: 식별 정보만 (날짜, 상태, 기수+이름+연락처)
...
const dropoutCount=data.filter(d=>d.join_status==='연락 후 미가입'||d.join_status==='상담 후 미가입'||d.join_status==='미가입').length;
const realDropoutRate=counseled>0?Math.round((dropoutCount/counseled)*100):0;
const applyDropoutRate=total>0?Math.round((dropoutCount/total)*100):0;
...
const counselorMap={};data.forEach(d=>{const cname=(d.counselor_name&&d.counselor_name!=='null'&&d.counselor_name.trim())?d.counselor_name.trim():'미지정';if(!counselorMap[cname])counselorMap[cname]={total:0,joined:0,dropout:0,pending:0};counselorMap[cname].total++;if(d.join_status==='가입 완료')counselorMap[cname].joined++;else if(d.join_status==='연락 후 미가입'||d.join_status==='상담 후 미가입'||d.join_status==='미가입')counselorMap[cname].dropout++;else counselorMap[cname].pending++;});
const counselorList=Object.entries(counselorMap).filter(([k,v])=>v.total>0&&k!=='미지정').sort((a,b)=>{const rA=a[1].total>0?a[1].joined/a[1].total:0;const rB=b[1].total>0?b[1].joined/b[1].total:0;return rB-rA;});
let channelMap={};let safeDataForSummary={instaFollow:0,instaNonFollow:0,adNow:0,leadTime3M:0};data.forEach(d=>{let rawAcq=String(d.acquisition_channel||'기타');let acq_ch=rawAcq;let etcDetail='';if(rawAcq.startsWith('기타')){let colonIdx=rawAcq.indexOf(':');if(colonIdx>-1){acq_ch='기타';etcDetail=rawAcq.substring(colonIdx+1).trim();}else{acq_ch='기타';}}if(!channelMap[acq_ch])channelMap[acq_ch]={total:0,details:{}};channelMap[acq_ch].total++;let detail='';if(acq_ch==='인스타그램'){if(d.is_follow==='네, 팔로우하고 있어요')safeDataForSummary.instaFollow++;else if(d.is_follow)safeDataForSummary.instaNonFollow++;detail=d.follow_duration||d.is_follow||'';}else if(acq_ch==='광고'){detail=d.ad_duration||'';if(detail==='최근 일주일 이내'||detail==='한 달 이내')safeDataForSummary.adNow++;else if(detail==='3개월 이내'||detail==='1년 이내')safeDataForSummary.leadTime3M++;}else if(acq_ch==='지인·카페·커뮤니티'||acq_ch==='지인 추천'){detail='';}else if(acq_ch==='기타'){detail=etcDetail;}if(detail){channelMap[acq_ch].details[detail]=(channelMap[acq_ch].details[detail]||0)+1;}});
let interestAllVals=[];let interestEtcMap={};data.forEach(d=>{if(d.interest_area)String(d.interest_area).split(',').map(s=>s.trim()).filter(Boolean).forEach(v=>{if(v.startsWith('기타')){let colonIdx=v.indexOf(':');if(colonIdx>-1){let etcDet=v.substring(colonIdx+1).trim();interestAllVals.push('기타');if(etcDet)interestEtcMap[etcDet]=(interestEtcMap[etcDet]||0)+1;}else{interestAllVals.push('기타');}}else{interestAllVals.push(v);}});});
let interestData=getFrequency(interestAllVals);let topInterest=interestData.length>0?interestData[0][0]:'없음';let topInterestRate=total>0&&interestData.length>0?Math.round((interestData[0][1]/total)*100):0;
...
// ★★★ PATCH: searchMembers가 만든 결과를 window.currentFilteredMembers에도 동시 갱신 → downloadExcel('members')에서 재사용 ★★★
...
// ★★★ PATCH (E): renderMemberTablePage 시작/끝에 체크박스 보존·복원 ★★★
// ★★★ PATCH (F): getCancelCount에서 samePhone 사용 → 010-xxxx vs 010xxxx 매칭 ★★★
window.renderMemberTablePage=function(){
    ...
    if(data.length===0){tbody.innerHTML=`<tr><td colspan="8" class="empty-state">내역이 없습니다.</td></tr>`;updatePaginationUI(0);try{if(_savedCheckboxes)window.restoreCheckboxState(_savedCheckboxes);}catch(e){}return;}
...
let amountStr='0원';let targetStatus='연장 활동 중';if(opt==='1'){baseDateForUpdate.setDate(baseDateForUpdate.getDate()+31);amountStr='220,000원';}else if(opt==='3'){baseDateForUpdate.setDate(baseDateForUpdate.getDate()+93);amountStr='550,000원';}else if(opt==='6'){baseDateForUpdate.setDate(baseDateForUpdate.getDate()+186);amountStr='1,100,000원';}else if(opt==='bonus'){baseDateForUpdate.setDate(baseDateForUpdate.getDate()+31);amountStr='무료 제공';}else if(opt==='day'){baseDateForUpdate.setDate(baseDateForUpdate.getDate()+1);amountStr='별도 안내';targetStatus='단일권 이용';}
...
window.deleteHistory=async function(id,phone,name,action_detail){window.openCustomConfirm("내역 삭제",null,`해당 내역을 완전히 삭제하시겠습니까?<br><span style='font-size:12px;color:var(--text-secondary);'>(삭제 시, 늘어난 종료일이 자동으로 계산되어 복구됩니다.)</span>`,async()=>{await supabaseClient.from('member_history').delete().eq('id',id);const m=globalMembers.find(x=>window.samePhone(x.phone,phone));if(m&&m.end_date){let d=new Date(m.end_date);let isChanged=false;if(action_detail.includes('1개월 연장')||action_detail.includes('보너스 1개월')){d.setDate(d.getDate()-31);isChanged=true;}else if(action_detail.includes('3개월 연장')){d.setDate(d.getDate()-93);isChanged=true;}else if(action_detail.includes('6개월 연장')){d.setDate(d.getDate()-186);isChanged=true;}else if(action_detail.includes('당일권 추가')){d.setDate(d.getDate()-1);isChanged=true;}if(isChanged){let newEndDate=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;m.end_date=newEndDate;await supabaseClient.from('members').update({end_date:newEndDate}).eq('phone',phone);}}showToast("내역이 삭제되고 종료일이 복구되었습니다.");window.searchMembers();window.openHistoryModal(phone,name);});};
...
// ============================================================
// ★★★ PATCH (메인 수정): downloadExcel — 현재 화면 필터를 그대로 export에 반영 ★★★
// - members: currentFilteredMembers (검색어/상태/기수 모두 반영)
// - applications: batchFilterApp 반영 (currentFilteredApps 사용)
// - reservations: currentFilteredRes (검색어/공간/센터 반영)
// - trainings: currentFilteredTrn (검색어/콘텐츠/센터 반영)
// - orders: 검색어/생두사/접수만보기/센터/만료 필터 전부 반영
// - 전화번호는 정규화(010-xxxx-xxxx) 포맷으로 출력
// ============================================================
window.downloadExcel = function(type) {
    try {
        // 인사이트 모드 export (applications + isInsightView)는 기존 로직 그대로
        ...
        if(type === 'applications') {
            // 기수 필터 반영: applyFilterApp이 캐싱한 결과 우선
            if(Array.isArray(window.currentFilteredApps)) {
                data = window.currentFilteredApps;
            } else {
            ...
            filename = "가입신청";
            headers = ['신청일','기수','성함','연락처','관심분야','관심도','유입경로','인지 기간','진행상황','가입여부','상담일시','담당자'];
        }
        else if(type === 'members') {
            // 검색어 + 상태 + 기수 필터 전부 반영
            const filterEmpty = (!$("memberSearch") || !$("memberSearch").value.trim())
                && (!$("memberStatusFilter") || $("memberStatusFilter").value === 'all')
                && (!$("memberBatchFilter") || $("memberBatchFilter").value === 'all');
            if(filterEmpty) {
                // 필터가 모두 비어 있으면 전체 노출
                data = globalMembers;
            } else {
                // 필터가 하나라도 걸려 있으면 그 결과만 (필터 결과가 0건이면 진짜 0건)
                data = (Array.isArray(window.currentFilteredMembers) ? window.currentFilteredMembers : currentFilteredMembers) || [];
            }
            ...
            // 검색어 + 공간 + 센터 필터 반영
            data = Array.isArray(window.currentFilteredRes) ? window.currentFilteredRes : gRes;
            filename = "예약현황";
            headers = ['접수일','기수','성함','연락처','예약날짜','예약시간','센터','장비','상태','취소사유'];
        }
        else if(type === 'trainings') {
            // 검색어 + 콘텐츠 + 센터 필터 반영
            data = Array.isArray(window.currentFilteredTrn) ? window.currentFilteredTrn : gTrn;
            filename = "수업훈련";
            headers = ['신청일','기수','성함','연락처','콘텐츠','상태','취소사유'];
        }
        else if(type === 'orders') {
            // renderCenterData와 동일 로직: 검색어 + 생두사 + 접수만보기 + 센터 + 만료 필터
            const now = new Date();
            ...
            data = gOrd.filter(o => {
                ...
                const notExpired = isOrdFilter ? true : (typeof window.isOrderExpired === 'function' ? !window.isOrderExpired(o, now) : true);
        ...
        if(!data || data.length === 0) {
            if(typeof showToast === 'function') showToast('다운로드할 데이터가 없습니다.');
            return;
        }
        ...
        data.forEach(d => {
            let row = [];
            // ★ 전화번호 정규화: 010-1234-5678 형식으로 통일 출력
            const phoneOut = window.normalizePhone(d.phone) || d.phone || '';
            if(type === 'applications')      row = [formatDt(d.created_at), d.desired_batch, d.name, phoneOut, d.interest_area, d.interest_level, d.acquisition_channel, d.known_duration, d.status, d.join_status, d.call_time, d.counselor_name];
            ...
            csvContent += row.map(item => {
                let text = String(item == null ? '' : item);
                text = text.replace(/"/g, '""');
                text = text.replace(/\n/g, ' ');
                return `"${text}"`;
            }).join(',') + '\n';
        ...
        if(typeof showToast === 'function') showToast('엑셀 다운로드 중 오류가 발생했습니다.');
    }
};
// ============================================================
// ★★★ downloadExcel 수정 끝 ★★★
...
// ★★★ PATCH (서버부하 ⑧ + F): 끝 4자리 ILIKE + 정규화 매칭 ★★★
window.openCrmModalFromPhone=async function(phone){
    ...
    if(app){
        ...
        const{data,error}=await supabaseClient
            ...
            .limit(50);
        if(!error&&data){
            let matched=data.find(a=>window.samePhone(a.phone, phone));
            if(matched){
...
window.cancelAction=function(table,id){
    ...
    if(table==='reservations'){const rec=gRes.find(r=>String(r.id)===String(id));if(rec)eventDateStr=rec.res_date;}
    else if(table==='trainings'){const rec=gTrn.find(t=>String(t.id)===String(id));if(rec){const cInfo=String(rec.content||'').split('||').map(s=>s.trim());if(cInfo[0]&&cInfo[0].match(/\d{4}-\d{2}-\d{2}/))eventDateStr=cInfo[0];}}
    const isSameDay=eventDateStr===todayStr;
    const cancelStatus=isSameDay?'당일 취소':'취소';
    window.openCustomConfirm("일정 취소",null,
        isSameDay?`<div style="background:#fff0f0;border:1px solid #ffcdd2;border-radius:6px;padding:10px 14px;font-size:13px;color:var(--error);font-weight:700;">⚠️ 오늘 일정입니다. 당일 취소로 처리됩니다.</div>`:`이 일정을 취소 처리하시겠습니까?`,
        async()=>{const{error}=await supabaseClient.from(table).update({status:cancelStatus}).eq('id',id);if(error)showToast("취소 처리 실패");else{showToast(isSameDay?"당일 취소 처리되었습니다.":"취소 처리되었습니다.");window.fetchCenterData({force:true});}});
};
...
window.saveBlockData=async function(){
    ...
    if(id&&id!==""){
        const oldBlock=gBlk.find(b=>String(b.id)===String(id));
        const newPayload=payloads[0];
        const res=await supabaseClient.from('blocks').update(newPayload).eq('id',id);
        error=res.error;
        if(!error&&oldBlock){
            try{
                const oldContentKey=`[${oldBlock.category}] ${oldBlock.reason}`;
                const oldTimeRange=`${oldBlock.start_time}~${oldBlock.end_time}`;
                const newContentKey=`[${newPayload.category}] ${newPayload.reason}`;
                const newTimeRange=`${newPayload.start_time}~${newPayload.end_time}`;
                const affected=gTrn.filter(t=>{const cInfo=String(t.content||'').split('||').map(s=>s.trim());if(cInfo.length<5)return false;return cInfo[0]===oldBlock.block_date&&cInfo[2]===oldTimeRange&&cInfo[3]===oldBlock.center&&cInfo[4]===oldContentKey;});
                for(const t of affected){
                    const cInfo=String(t.content||'').split('||').map(s=>s.trim());
                    cInfo[0]=newPayload.block_date;cInfo[2]=newTimeRange;cInfo[3]=newPayload.center;cInfo[4]=newContentKey;
                    const newContent=cInfo.join(' || ');
                    await supabaseClient.from('trainings').update({content:newContent}).eq('id',t.id);
                }
                syncResult.updated=affected.length;
                const changes=[];
                if(oldBlock.block_date!==newPayload.block_date)changes.push(`날짜: ${oldBlock.block_date} → ${newPayload.block_date}`);
                if(oldTimeRange!==newTimeRange)changes.push(`시간: ${oldTimeRange} → ${newTimeRange}`);
                if(oldBlock.center!==newPayload.center)changes.push(`센터: ${oldBlock.center} → ${newPayload.center}`);
                if((oldBlock.space_equip||'전체')!==(newPayload.space_equip||'전체'))changes.push(`공간/장비: ${oldBlock.space_equip||'전체'} → ${newPayload.space_equip||'전체'}`);
                if(oldBlock.reason!==newPayload.reason)changes.push(`상세 내용: ${oldBlock.reason} → ${newPayload.reason}`);
                const activeAffected=affected.filter(t=>!String(t.status||'').includes('취소'));
                if(activeAffected.length>0&&changes.length>0){
                    const changeText=changes.join('\n');
                    const notifications=activeAffected.map(t=>({
                        member_phone:t.phone,
                        member_name:t.name,
                        title:'신청하신 수업/훈련 정보가 변경되었습니다',
                        message:`${t.name}님이 신청하신 [${oldBlock.reason}]의 정보가 다음과 같이 변경되었습니다.\n\n${changeText}\n\n일정 확인 후 참여가 어려울 경우 취소 요청드립니다.`,
                        related_type:'training_change',
                        related_id:String(id)
                    }));
                    const notifRes=await supabaseClient.from('member_notifications').insert(notifications);
                    if(!notifRes.error)syncResult.notified=activeAffected.length;
                }
            }catch(syncErr){console.warn('Sync error:',syncErr);}
        }
    }else{
    ...
    if(error){showToast("저장 실패");console.error(error);}
        ...
        let msg=payloads.length>1?`${payloads.length}개의 스케줄이 등록되었습니다.`:"저장되었습니다.";
        if(syncResult.updated>0){msg=`저장 완료. 신청자 ${syncResult.updated}명의 정보가 함께 업데이트되었습니다${syncResult.notified>0?` (${syncResult.notified}명에게 알림 발송)`:''}.`;}
        showToast(msg);
...
// ★★★ PATCH (서버부하 ⑨): saveNoticeData에 force:true ★★★
