...
wecoffeeStyle.innerHTML = `
    ...
    .edit-schedule-link { font-size:12px; font-weight:500; color:var(--text-secondary); cursor:pointer; margin-top:6px; transition:all 0.12s; }
    .edit-schedule-link:hover { font-weight:700; color:var(--text-display); }
...
window.downloadExcel = function(type) {
    try {
        ...
        data.forEach(d => {
            ...
            if(type === 'applications')      row = [formatDt(d.created_at), d.desired_batch, d.name, phoneOut, d.interest_area, window.mapInterestLevel(d.interest_level), d.acquisition_channel, d.known_duration, d.status, d.join_status, d.call_time, d.counselor_name];
