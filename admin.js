...
wecoffeeStyle.innerHTML = `
    ...
    @media (max-width: 768px) { 
        ...
        .timeline-mobile { display: flex !important; flex-direction: column; gap: 12px; width: 100%; }
        
        /* 모바일 카드 리스트 스타일 */
        .m-time-card { background: #fff; border: 1px solid #eee; border-radius: 12px; padding: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.02); display: flex; flex-direction: column; gap: 8px; border-left: 4px solid var(--border-strong); }
        .m-time-card.type-res { border-left-color: var(--primary); }
        .m-time-card.type-trn { border-left-color: rgba(255, 121, 0, 0.65); }
        .m-time-card.type-blk { border-left-color: #9ca3af; }
        .m-time-header { display: flex; justify-content: space-between; align-items: center; }
        .m-time-badge { font-size: 16px; font-weight: 800; color: var(--text-display); letter-spacing: -0.5px; }
        .m-equip-badge { font-size: 11px; padding: 4px 8px; background: #f4f5f7; color: #505967; border-radius: 6px; font-weight: 700; word-break: keep-all; }
        .m-user-info { font-size: 14px; font-weight: 600; color: var(--text-display); line-height: 1.4; word-break: keep-all;}
        .m-sub-info { font-size: 13px; color: var(--text-secondary); margin-top: 2px;}
    }
`;
        ...
        wrapper.querySelectorAll('.space-opt-item').forEach(item => {
            item.addEventListener('click', function(e) {
                ...
                if(clickedVal === '전체 (공간 전체)') { blkSpaceInput.value = '전체 (공간 전체)'; } 
                    ...
                    // 💡 해결: 불필요하게 남던 꼬리 콤마(,) 제거. 단일 선택 시 깔끔하게 처리됩니다.
...
function startRealtimeSync() {
    ...
    // 💡 해결: 웹소켓 장애/미설정 시에도 화면을 백그라운드에서 강제 갱신하는 30초 폴링(Polling) 이중 안전장치
    if (fallbackSyncInterval) clearInterval(fallbackSyncInterval);
    fallbackSyncInterval = setInterval(() => {
        let activeMain = localStorage.getItem('wecoffee_main_tab') || 'page-center';
        if (activeMain === 'page-center') window.fetchCenterData();
        else if (activeMain === 'page-applications') window.fetchApplications();
        else if (activeMain === 'page-members') window.fetchMembers();
    }, 30000);
}
...
function initializeApp() {
  ...
  supabaseClient.auth.onAuthStateChange((event, session) => {
    if (session) { if(!isAppInitialized) { handleLoginSuccess(); isAppInitialized = true; } } 
        ...
        // 💡 해결: 로그아웃 시 폴링 타이머 중지
        if(fallbackSyncInterval) { clearInterval(fallbackSyncInterval); fallbackSyncInterval = null; }
    }
  });
}
...
// 💡 해결: 누락되었던 메인 대시보드(센터 일정) 뷰 전환 및 월 이동 제어 함수 복구
...
window.renderTimeline = function() {
    ...
    // 💡 모바일 뷰: 세로형 카드 리스트 렌더링 로직 (제안 2)
    let mobileEvents = [];
    gRes.forEach(r => {
        if (r.res_date === todayStr && centersToRender.includes(r.center) && !String(r.status).includes('취소')) {
            mobileEvents.push({ type: 'res', center: r.center, time: r.res_time, start: r.res_time.split('~')[0].trim(), equip: r.space_equip, name: `[${r.batch||'-'}] ${r.name}` });
        }
    });
    gTrn.forEach(t => {
        let cInfo = String(t.content || '').split('||').map(s => s.trim());
        if (cInfo.length >= 5 && cInfo[0] === todayStr && centersToRender.includes(cInfo[3]) && !String(t.status).includes('취소')) {
            mobileEvents.push({ type: 'trn', center: cInfo[3], time: cInfo[2], start: cInfo[2].split('~')[0].trim(), equip: cInfo[4], name: `[수강] ${t.name}` });
        }
    });
    gBlk.forEach(b => {
        if (b.block_date === todayStr && centersToRender.includes(b.center)) {
            mobileEvents.push({ type: 'blk', center: b.center, time: `${b.start_time}~${b.end_time}`, start: b.start_time, equip: b.space_equip || '공간 전체', name: `[${b.category}] ${b.reason}` });
        }
    });

    mobileEvents.sort((a, b) => {
        let aMins = parseInt(a.start.split(':')[0]||0) * 60 + parseInt(a.start.split(':')[1]||0);
        let bMins = parseInt(b.start.split(':')[0]||0) * 60 + parseInt(b.start.split(':')[1]||0);
        return aMins - bMins;
    });

    let mobileHtml = `<div class="timeline-mobile">`;
    if (mobileEvents.length === 0) {
        mobileHtml += `<div class="empty-state" style="padding: 40px 0; background:#f9fafb; border-radius:12px; border:1px solid #eee;">오늘 예정된 일정이 없습니다.</div>`;
    } else {
        mobileEvents.forEach(e => {
            let typeClass = e.type === 'res' ? 'type-res' : (e.type === 'trn' ? 'type-trn' : 'type-blk');
            let timeBadgeColor = e.type === 'res' ? 'var(--primary)' : (e.type === 'trn' ? 'rgba(255, 121, 0, 0.9)' : '#6b7280');
            mobileHtml += `
                <div class="m-time-card ${typeClass}">
                    <div class="m-time-header">
                        <div class="m-time-badge" style="color: ${timeBadgeColor};">${e.time}</div>
                        <div class="m-equip-badge">${e.center} | ${e.equip}</div>
                    </div>
                    <div class="m-user-info">${window.escapeHtml(e.name)}</div>
                </div>`;
        });
    }
    mobileHtml += `</div>`;
    ...
    // 💡 PC 뷰: 가로 2D 그리드 (좌측 텍스트 Sticky 적용) 렌더링 로직
    let desktopHtml = `<div class="timeline-desktop">`;
    ...
    centersToRender.forEach((centerName, idx) => {
        ...
        spaceGroups.forEach(group => {
            if (group.equips.length === 1 && group.equips[0] === 'merged') {
                desktopHtml += `<div class="timeline-row"><div class="merged-col">${group.zone}</div><div class="time-grid-bg">${renderBarsFor('merged', group.zone, centerName)}</div></div>`;
            } else {
                desktopHtml += `<div class="zone-group-row"><div class="zone-col">${group.zone}</div><div class="equip-col-wrapper">`;
                group.equips.forEach(eq => {
                    desktopHtml += `<div class="timeline-row"><div class="equip-name">${eq}</div><div class="time-grid-bg">${renderBarsFor(eq, group.zone, centerName)}</div></div>`;
                });
                desktopHtml += `</div></div>`;
            }
        });
    ...
    // 모바일 HTML과 데스크탑 HTML 통합 렌더링
    finalHtml += mobileHtml + desktopHtml + `</div>`;
    timelineArea.innerHTML = finalHtml;
};
...
window.renderAppDashboard = async function() {
    ...
    window.appCalEvts = calEvts; let mobStrip = `<div class="mobile-cal"><div class="m-cal-strip" id="m-cal-strip-app">`; 
    iterDates.forEach(ds => { let dObj = new Date(ds); let dayKr = ["일","월","화","수","목","금","토"][dObj.getDay()]; let hasEvt = calEvts[ds].length > 0 ? 'has-evt' : ''; mobStrip += `<div class="m-cal-date" id="m-date-app-${ds}" onclick="window.renderMCalApp('${ds}')"><span class="m-cal-day">${dayKr}</span><span class="m-cal-num">${dObj.getDate()}</span><div class="m-cal-dot ${hasEvt}"></div></div>`; }); 
...
window.handleMemberOption = function(id, batch, name, phone, currentEndDate, selectEl) {
  ...
  window.openCustomConfirm(`[${batch || '미정'}] ${name} 님`, statText, confirmMsg, async () => {
      ...
      if (opt === '1') { baseDateForUpdate.setDate(baseDateForUpdate.getDate() + 31); amountStr = '220,000원'; } 
      else if (opt === '3') { baseDateForUpdate.setDate(baseDateForUpdate.getDate() + 93); amountStr = '550,000원'; } 
      else if (opt === '6') { baseDateForUpdate.setDate(baseDateForUpdate.getDate() + 186); amountStr = '1,100,000원'; } 
      else if (opt === 'bonus') { baseDateForUpdate.setDate(baseDateForUpdate.getDate() + 31); amountStr = '무료 제공'; } 
...
window.deleteHistory = async function(id, phone, name, action_detail) { 
    window.openCustomConfirm("내역 삭제", null, `해당 내역을 완전히 삭제하시겠습니까?<br><span style='font-size:12px;color:var(--text-secondary);'>(삭제 시, 늘어난 종료일이 자동으로 계산되어 복구됩니다.)</span>`, async () => { 
        ...
        if (m && m.end_date) { 
            ...
            let isChanged = false; 
            if (action_detail.includes('1개월 연장') || action_detail.includes('보너스 1개월')) { d.setDate(d.getDate() - 31); isChanged = true; } 
            else if (action_detail.includes('3개월 연장')) { d.setDate(d.getDate() - 93); isChanged = true; } 
            else if (action_detail.includes('6개월 연장')) { d.setDate(d.getDate() - 186); isChanged = true; } 
...
window.showOrderSummary = function() {
    ...
    if (pendingOrders.length === 0) {
        ...
        pendingOrders.forEach(o => {
            ...
            let safeBatch = (!o.batch || String(o.batch).trim() === 'undefined') ? '-' : String(o.batch);

            grouped[bigKey][vendor].items[cNm].orderers.push({ batch: safeBatch, name: safeName, phone: safePhone, rawQty: o.quantity || '0' });
        });
        ...
        sortedBigKeys.forEach(bigKey => {
            ...
            sortedVendors.forEach(vendor => {
                ...
                sortedItems.forEach(item => {
                    ...
                    let displayQty = d.totalGrams >= 1000 ? (d.totalGrams % 1000 === 0 ? (d.totalGrams / 1000) + 'kg' : (d.totalGrams / 1000) + 'kg') : d.totalGrams + 'g';
                    ...
                    html += `<div style="margin: 12px 0; padding: 16px; background:#fff; border:1px solid var(--border-strong); border-radius:12px;">
                        <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                            ...
                            <div style="font-size:20px; font-weight:900; color:var(--primary); margin-left:12px; white-space:nowrap;">${displayQty}</div>
                        </div>
                        <div style="font-size:12px; color:var(--text-tertiary); margin-top:8px; line-height:1.5;">주문자: ${window.escapeHtml(ordererText)}</div>
                        <div style="margin-top:10px; text-align:right;"><span style="font-size:11px; color:var(--primary); cursor:pointer; font-weight:800; border:1px solid var(--primary); padding:4px 10px; border-radius:6px;" onclick="window.copyTxt('${String(item).replace(/'/g, "\\'")}', '상품명이 복사되었습니다.')">상품명 복사</span></div>
                    </div>`;
                });
                
                let vTotalQty = vData.totalGrams >= 1000 ? (vData.totalGrams % 1000 === 0 ? (vData.totalGrams / 1000) + 'kg' : (vData.totalGrams / 1000) + 'kg') : vData.totalGrams + 'g';
                vTotalQty = vTotalQty.replace('.0kg', 'kg');
                html += `<div style="margin-bottom: 24px; padding: 16px; background: #f9fafb; border-radius: 12px; display: flex; justify-content: space-between; align-items: center; border: 1px solid #e5e8eb;">
                    <span style="font-size: 13px; font-weight: 600; color: var(--text-secondary);">${window.escapeHtml(vendor)} 선택된 발주 총 수량</span>
                    <span style="font-size: 18px; font-weight: 900; color: var(--text-display);">${vTotalQty}</span>
                </div>`;
            });
        });
        
        html += `<div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid var(--border); display:flex; justify-content:center; gap:8px;">
            <button class="btn-outline" style="border-color:#32b06a; color:#32b06a; padding:12px 24px; font-size:14px; font-weight:700;" id="btn-send-sheet" onclick="window.sendToGoogleSheet()">구글 시트 전송</button>
            <button class="btn-primary" style="padding: 12px 24px; font-size: 14px; font-weight:700; background:var(--primary); color:#fff; border:none; border-radius:8px;" onclick="window.downloadSummaryExcel()">엑셀 다운로드</button>
        </div></div>`;
        ...
        exportData.sort((a, b) => {
            if(a["발주 구분"] !== b["발주 구분"]) return a["발주 구분"].localeCompare(b["발주 구분"]);
            if(a["수령 센터"] !== b["수령 센터"]) return a["수령 센터"].localeCompare(b["수령 센터"]);
...
window.sendToGoogleSheet = async function() {
    ...
    try {
        ...
        let invoiceData = uniqueMembers.map(name => {
            let info = window.currentMemberInfoMap && window.currentMemberInfoMap[name] ? window.currentMemberInfoMap[name] : { phone: '', batch: '' };
            return {
                "등록 일시": "", "발주 구분": "", "수령 센터": "", "생두사": "",
                "상품명": `[${name}] 님 최종 청구 금액`, "주문 수량": "", 
                "결제 금액": `CALC_TOTAL:${name}`,
                "기수": info.batch, "성함": name, "연락처": info.phone
            };
        });
...
window.downloadSummaryExcel = function() {
    ...
    window.currentSummaryData.forEach(s => { 
        let dt = s['등록 일시'] || ''; let type = s['발주 구분'] || ''; let center = s['수령 센터'] || ''; let vendor = s['생두사'] || ''; let item = String(s['상품명'] || '').replace(/"/g, '""'); let qty = s['주문 수량'] || ''; let price = s['결제 금액'] || ''; let batch = String(s['기수'] || ''); let name = s['성함'] || ''; let phone = s['연락처'] || '';
    ...
    if(uniqueMembers.length > 0) {
        ...
        uniqueMembers.forEach(name => {
            let info = window.currentMemberInfoMap && window.currentMemberInfoMap[name] ? window.currentMemberInfoMap[name] : { phone: '', batch: '' };
            csv += `,,,,"[${name}] 님 최종 청구 금액",,"=IFERROR(SUMIFS(G2:G${dataEndRow}, I2:I${dataEndRow}, ""${name}""), 0)","${info.batch}","${name}","${info.phone}"\n`;
        });
    }
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' }); 
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); 
    link.download = `위커피_발주명단_및_청구서_${new Date().toISOString().slice(0,10)}.csv`; link.click(); 
};
...
window.editBlock = function(id) {
    ...
    if($("blkSpace")) {
        ...
        $("blkSpace").dataset.selectedValues = b.space_equip || '';
    }
