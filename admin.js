...
// 💡 수정사항: 타임라인 CSS를 HTML 문자열이 아닌, 가장 안전한 글로벌 <head> 영역으로 분리시켜 디자인이 깨지지 않게 보장합니다.
if (!document.getElementById('wecoffee-custom-styles')) {
    ...
    style.innerHTML = `
        ...
        /* 1단 보장 및 CSS 깨짐 방지용 타임라인 스타일 세팅 */
        #timeline-area { width: 100%; max-width: 100vw; box-sizing: border-box; margin-top: 32px; display: block; clear: both; }
        #timeline-area .timeline-section { width: 100%; background: #fff; padding: 24px; border-radius: 12px; border: 1px solid var(--border-strong); box-shadow: 0 4px 20px rgba(0,0,0,0.05); box-sizing: border-box; overflow: hidden; text-align: left; }
        .timeline-container { width: 100%; overflow-x: auto; position: relative; border: 1px solid #eee; border-radius: 8px; -webkit-overflow-scrolling: touch; padding-bottom: 8px; box-sizing: border-box; text-align: left; }
        ...
        .timeline-header { display: flex; flex-direction: row; background: #f9fafb; border-bottom: 2px solid #eee; }
        ...
        .time-slots-header { display: flex; flex-direction: row; flex-grow: 1; }
        ...
        .zone-group-row { display: flex; flex-direction: row; border-bottom: 1px solid #eee; }
        ...
        .timeline-row { display: flex; flex-direction: row; border-bottom: 1px solid #eee; min-height: 54px; position: relative; }
        ...
        .time-grid-bg { display: flex; flex-direction: row; flex-grow: 1; position: relative; background-image: repeating-linear-gradient(to right, transparent, transparent calc(4.16666% - 1px), #f0f0f0 calc(4.16666% - 1px), #f0f0f0 4.16666%); }
        ...
        @media (max-width: 768px) { .mem-action-wrap { flex-direction: column !important; align-items: stretch !important; gap: 8px !important; width: 100%; overflow-x: visible; } .mem-action-row { width: 100%; justify-content: space-between; flex-wrap: wrap !important; gap: 6px; } .mem-action-row select { flex: 1; min-width: 0; padding-left: 8px !important; padding-right: 28px !important; } #timeline-area .timeline-section { padding: 16px; margin-bottom: 24px !important; } }
    `;
...
window.fetchCenterData = async function() {
  ...
  // 💡 수정된 부분: 타임라인(100%)을 출입현황 배너(5:5) 부모 영역 바깥으로 완전히 빼내어 삽입
  try {
...
    // 💡 수정사항: 인라인 <style> 태그를 제거하고, Part 1의 전역 CSS를 활용하여 렌더링 충돌 및 깨짐 방지
    let finalHtml = `
