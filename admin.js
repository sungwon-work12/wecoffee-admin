...
window.renderTimeline = function() {
    ...
    // 💡 [초심 픽스] 가장 안정적인 문자열 포함 여부 매칭 (로스팅존 번호 중복 픽스 포함)
    function isMatch(dbSpace, uiEquip, zoneName) {
        let dbStr = String(dbSpace || '').trim();
        if (uiEquip === 'merged' || uiEquip === '공간 전체') {
            return dbStr === zoneName || dbStr.includes('전체') || dbStr === '전체 (공간 전체)';
        }
        // UI 장비명에서 (좌), (우) 괄호를 제거한 앞부분만 뽑아서 DB값에 있는지 확인
        let uiClean = uiEquip.split('(')[0].trim(); 
        
        // 깐깐한 조건: 기기명(예: 이지스터 800 1번)이 DB 문자열 안에 '그대로' 있어야 함
        // 로스팅존의 경우 '1번'과 '2번'을 확실히 구분하기 위해 uiEquip 풀네임 또는 uiClean으로 비교
        return dbStr.includes(uiClean); 
    }
