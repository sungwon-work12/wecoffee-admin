...
window.renderCrmInner=function(id,isReadOnly=false){const app=globalApps.find(a=>String(a.id)===String(id));if(!app)return;let cCount=0;let now=new Date();let monthPrefix=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;if(gRes&&gTrn){gRes.forEach(r=>{if(window.samePhone(r.phone,app.phone)&&r.status==='당일 취소'&&String(r.res_date||r.created_at).startsWith(monthPrefix))cCount++;});gTrn.forEach(t=>{if(window.samePhone(t.phone,app.phone)&&t.status==='당일 취소'){let dStr=String(t.content||'').split(' || ')[0]||String(t.created_at);if(dStr.startsWith(monthPrefix))cCount++;}});}let warnHtml=cCount>=4?`<span style="background:var(--error);color:#fff;font-size:11px;padding:2px 6px;border-radius:4px;margin-left:8px;font-weight:700;vertical-align:middle;">경고</span>`:'';if($("crmName"))$("crmName").innerHTML=`<span style="font-weight:800;color:var(--text-display);margin-right:4px;">[${app.desired_batch||'미정'}]</span> ${window.escapeHtml(app.name||'이름 없음')} ${warnHtml}`;// ★ 요청1: 프로필을 라벨 폭 고정 그리드로 재구성
    ...
    if(app.acquisition_channel) _profileHtml += _row('유입', window.escapeHtml(app.acquisition_channel));
    if(app.desired_center) _profileHtml += _row('희망센터', window.escapeHtml(app.desired_center));
    ...
    if(app.interest_area) _profileHtml += _row('관심분야', window.escapeHtml(app.interest_area), 'font-weight:600;color:var(--text-display);font-size:13px;line-height:1.5;');
...
window.renderAppTable = function(data) {
    ...
    data.forEach(row=>{
        ...
        // ★ NEW: interest_level + interest_area를 모바일 카드에 라벨과 함께 표시
        let levelHtml = row.interest_level ? `<div style="font-size:12px;margin-top:4px;line-height:1.3;"><span style="color:var(--text-tertiary);font-weight:600;">관심도</span>&nbsp;<span style="font-weight:700;color:var(--primary);">${window.escapeHtml(row.interest_level)}</span></div>` : '';
        let areaHtml = row.interest_area ? `<div style="font-size:12px;margin-top:2px;line-height:1.4;"><span style="color:var(--text-tertiary);font-weight:600;">관심분야</span>&nbsp;<span style="font-weight:600;color:var(--text-secondary);">${window.escapeHtml(row.interest_area)}</span></div>` : '';
        ...
        let _mTimeHtml = _mRawTime ? `<div style="font-size:12px;margin-top:8px;padding-top:8px;border-top:1px solid var(--border-strong,#e5e8eb);white-space:nowrap;"><span style="color:var(--text-secondary);font-weight:700;">${_mTimeLabel}</span>&nbsp;<span style="color:var(--text-display);font-weight:600;">${window.escapeHtml(_mRawTime)}</span></div>` : '';
        let mPreview=`<td class="m-preview" onclick="this.closest('tr').classList.toggle('expanded')"><div class="m-prev-top"><span class="m-prev-date">${formatDtWithDow(row.created_at)}</span><span class="status-badge ${cStat}">${row.status}</span></div><div class="m-prev-title">[${row.desired_batch||'-'}] ${window.escapeHtml(row.name)} <span style="font-size:13px;color:var(--text-secondary);">(${window.escapeHtml(window.normalizePhone(row.phone)||row.phone||'')})</span></div>${levelHtml}${areaHtml}${_mTimeHtml}</td>`;
