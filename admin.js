...
window.renderAppTable = function(data) {
    ...
    data.forEach(row=>{
        ...
        let _mTimeValue = '미정';
        if(_mRawTime){
            if(_mIsScheduled){let _cp=_mRawTime.split(' ');let _dn='',_tn='';if(_cp[0]){let _dp=_cp[0].split('-');let _dObj=new Date(parseInt(_dp[0]),parseInt(_dp[1])-1,parseInt(_dp[2]));let _dow=['일','월','화','수','목','금','토'][_dObj.getDay()];_dn=`${_dp[0]}년 ${parseInt(_dp[1])}월 ${parseInt(_dp[2])}일(${_dow})`;}if(_cp[1]){let _tp=_cp[1].split(':');let _h=parseInt(_tp[0]),_m=_tp[1]||'00';let _ap=_h>=12?'오후':'오전';let _h12=_h%12||12;_tn=` ${_ap} ${_h12}시${_m==='00'?'':' '+_m+'분'}`;}
            _mTimeValue=(_dn+_tn).trim();}else{_mTimeValue=_mRawTime;}
        }
