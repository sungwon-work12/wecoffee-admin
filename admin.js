...
window.openBlockModal = function() {
  ...
  $("blkId").value = ""; $("blkCategory").value = "기본 수업"; $("blkDate").value = ""; $("blkStart").value = ""; $("blkEnd").value = "";
...
window.editBlock = function(id) {
  ...
  $("blkId").value = b.id; $("blkCategory").value = b.category; $("blkDate").value = b.block_date; 
...
window.saveBlockData = async function() {
  const payload = { 
      category: $("blkCategory").value, block_date: window.formatBlockDate($("blkDate").value), start_time: window.formatBlockTime($("blkStart").value), 
      end_time: window.formatBlockTime($("blkEnd").value), center: $("blkCenter").value, space_equip: $("blkSpace").value.trim(), reason: $("blkReason").value.trim(), capacity: parseInt($("blkCapacity").value) || null 
  };
...
window.deleteBlock = function(id) {
  window.openCustomConfirm("스케줄 삭제", null, `해당 스케줄을 삭제하시겠습니까?`, async () => {
    const { error } = await supabaseClient.from('blocks').delete().eq('id', id); if(error) showToast("삭제 실패"); else { showToast("삭제되었습니다."); window.fetchCenterData(); }
  });
}
...
window.updateAppStatus = async function(id, column, value) {
    ...
    if (column === 'join_status' && value === '가입 완료') {
        window.openCustomConfirm("가입 완료 (멤버 전환)", null, `해당 고객을 멤버 리스트로 이관하시겠습니까?<br><span style="font-size:12px; color:var(--text-secondary);">오늘 기준으로 6개월 활동 종료일 및 결제 내역이 자동 세팅됩니다.</span>`, async () => {
...
window.renderAppTable = function(data) {
  ...
  data.forEach(row => {
    ...
    if(row.status === '상담 일정 확정' || row.status === '설문 완료') {
      ...
      emptyJoinSpace = `<span style="display:inline-block; font-size:12px; color:transparent; pointer-events:none; position:absolute; bottom:-20px;">-</span>`;
    }
