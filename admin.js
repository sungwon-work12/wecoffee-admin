...
// ★ 수정: 금액 변경 시 order_price_logs에 자동 기록
window.handlePriceInput = async function(id, val, currentStatus, inputEl) {
    ...
    if(oldPrice !== formatted){try{await supabaseClient.from('order_price_logs').insert([{order_id:String(id),member_name:order?.name||'',old_price:oldPrice,new_price:formatted,changed_by:currentAdminEmail||'unknown'}]);}catch(e){console.warn('price log failed',e);}}
    showToast("저장되었습니다.");
}
...
window.showPriceChangeLogs=async function(){
    ...
    modal.style.display='flex';
    try{
        const{data,error}=await supabaseClient.from('order_price_logs').select('*').order('created_at',{ascending:false}).limit(200);
        const body=document.getElementById('priceLogBody');
        if(error||!data||data.length===0){
            body.innerHTML='<div class="empty-state" style="padding:40px 0;color:var(--text-tertiary);">금액 변경 이력이 없습니다.</div>';return;
        }
        let rows=data.map(log=>{
            return `<tr><td style="padding:10px 12px;font-size:12px;color:var(--text-secondary);">${formatDt(log.created_at)}</td><td style="padding:10px 12px;font-weight:700;">${window.escapeHtml(log.member_name||'-')}</td><td style="padding:10px 12px;color:var(--text-tertiary);">${window.escapeHtml(log.old_price||'(없음)')}</td><td style="padding:10px 12px;font-weight:700;color:var(--primary);">${window.escapeHtml(log.new_price||'(없음)')}</td><td style="padding:10px 12px;font-size:12px;color:var(--text-secondary);">${window.escapeHtml(log.changed_by||'-')}</td></tr>`;
        }).join('');
        ...
        document.getElementById('priceLogBody').innerHTML='<div class="empty-state" style="color:var(--error);">이력 조회 중 오류가 발생했습니다.</div>';
        console.error(e);
    }
};
...
window.printSummary=function(){
    const modalBody=$("summaryModalBody");
    if(!modalBody){showToast("인쇄할 내용이 없습니다.");return;}
    const printWindow=window.open('','_blank','width=800,height=600');
    printWindow.document.write(`<!DOCTYPE html><html><head><title>발주 요약 인쇄</title><style>body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;padding:24px;color:#191f28;font-size:13px;line-height:1.6;}button{display:none !important;}*{-webkit-print-color-adjust:exact;print-color-adjust:exact;}</style></head><body>${modalBody.innerHTML}</body></html>`);
