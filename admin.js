window.showOrderSummary = function() {
    ...
    if (pendingOrders.length === 0) {
        ...
        pendingOrders.forEach(o => {
            ...
            // 💡 [핵심 수정] 시트 전송에 필요한 모든 개별 항목을 누락 없이 담음
            summary[key].orderers.push({ 
                createdAt: o.created_at, // 등록 일시용
                batch: o.batch || '미정', 
                name: o.name, 
                phone: o.phone, 
                rawQty: o.quantity || '0' 
        ...
        sortedData.forEach(s => {
            ...
            let displayQty = s.totalGrams >= 1000 ? (s.totalGrams / 1000) + 'kg' : s.totalGrams + 'g';
...
// 💡 [구글 시트 전송 오류 해결] 주문 시간 데이터 조립 및 정확한 Key 매핑 완료
window.showOrderSummary = function() {
    let qOrd = ($("searchOrd")?.value || "").toLowerCase();
    let vOrd = $("ordVendorFilter")?.value || "전체";
    ...
    if (pendingOrders.length === 0) {
        ...
        pendingOrders.forEach(o => {
            ...
            // 💡 [수정] 주문 시간(created_at)을 추출하여 orderers 배열에 함께 저장
            summary[key].orderers.push({ 
        ...
        sortedData.forEach(s => { 
            s.orderers.forEach(o => { 
                // 💡 [수정] 주문 시간 등 스프레드시트 헤더 요구사항에 맞게 명칭과 데이터를 완벽히 매핑
                exportData.push({ 
...
// 💡 [긴급 수정] 날짜 파싱 오류로 인한 모달창 기능 정지(먹통) 원인 제거용 포맷터 강제 재정의
window.formatCounselDateDisplay = function(val) {
    ...
    let now = new Date();
    if (dt.length >= 8) {
        y = parseInt(dt.slice(0, 4), 10);
        m = parseInt(dt.slice(4, 6), 10);
        d = parseInt(dt.slice(6, 8), 10);
    } else if (dt.length === 4) {
        y = now.getFullYear();
        m = parseInt(dt.slice(0, 2), 10);
        d = parseInt(dt.slice(2, 4), 10);
...
window.formatCounselDateRaw = function(val) {
    if(!val) return '';
    let match = val.match(/(\d{4})년\s*(\d{1,2})월\s*(\d{1,2})일/);
    if(match) return `${match[1]}-${String(match[2]).padStart(2, '0')}-${String(match[3]).padStart(2, '0')}`;
    let matchOld = val.match(/(\d+)년\s*(\d+)월\s*(\d+)일/);
    if(matchOld) return String(matchOld[2]).padStart(2,'0') + String(matchOld[3]).padStart(2,'0');
    let dt = String(val).replace(/\D/g, '');
    if(dt.length > 4 && dt.length !== 8) return dt.slice(-4);
    return dt;
};
