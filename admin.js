...
window.renderCenterData = function() {
  ...
  $("resTableBody").innerHTML = fRes.length ? fRes.map(r=>{ 
    ...
    let actBtn = displayStatus.includes('취소') ? '' : `<button class="btn-outline btn-sm" onclick="window.cancelAction('reservations', '${r.id}')">취소</button>`; 
    let endTimeStr = String(r.res_time||"").split('~')[1];
    if (endTimeStr && r.res_date && !displayStatus.includes('취소')) {
  ...
  let fTrnList = gTrn.filter(t => { let tDate = new Date(t.created_at); return (tDate >= oneMonthAgo) && (currentGlobalCenter === '전체' || String(t.content||"").includes(currentGlobalCenter)) && (`${t.name} ${t.phone} ${t.content}`.toLowerCase().includes(qTrn)); });
  
  $("trnTableBody").innerHTML = fTrnList.length ? fTrnList.map(t=>{ 
    let displayStatus = t.status || '';
    let actBtn = displayStatus.includes('취소') ? '' : `<button class="btn-outline btn-sm" onclick="window.cancelAction('trainings', '${t.id}')">취소</button>`; 
    ...
    let badgeClass = displayStatus.includes('취소')?'badge-red':displayStatus==='접수완료'?'badge-green':'badge-gray';
  ...
  // 🔥 생두 데이터 글로벌 센터 필터링 로직 수정 (마포/광진 토글 연동)
