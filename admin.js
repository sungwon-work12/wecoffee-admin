...
function formatDt(dateStr) { if(!dateStr) return "-"; try { let str=String(dateStr).replace('T',' ').split('.')[0]; let parts=str.split(' '); if(parts.length<2) return str; let [y,m,d]=parts[0].split('-'); let [hh,mm]=parts[1].split(':'); if(!y||!m||!d||!hh||!mm) return str; let dow=['일','월','화','수','목','금','토'][new Date(y,m-1,d).getDay()]||''; let h=parseInt(hh),mi=parseInt(mm); let ap=h>=12?'오후':'오전'; let h12=h%12||12; let miStr=mi>0?` ${mi}분`:''; let yearPrefix=String(y)===String(new Date().getFullYear())?'':`${y}년 `; return `${yearPrefix}${parseInt(m)}/${parseInt(d)}(${dow}) ${ap} ${h12}시${miStr}`; } catch(e) { return String(dateStr); } }
