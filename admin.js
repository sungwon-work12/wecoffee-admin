    ...
    statusConfig.forEach((cfg,gi)=>{
        ...
        if(cfg.items.length===0){
            ...
            cfg.items.forEach(row=>{
                ...
                accHtml+=`<div style="background:#fff;border:1px solid var(--border-strong);border-radius:10px;padding:14px 16px;margin-bottom:6px;cursor:pointer;transition:0.1s;" onmouseover="this.style.borderColor='var(--text-tertiary)'" onmouseout="this.style.borderColor='var(--border-strong)'" onclick="window.openCrmModal('${row.id}')">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
                        <div style="flex:1;min-width:0;">
                            ...
                            <div style="font-size:13px;color:var(--text-secondary);display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                                <span>${window.escapeHtml(window.normalizePhone(row.phone)||row.phone||'')}</span>
                                <span style="color:var(--border-strong);">·</span>
                                <span>${window.escapeHtml(row.acquisition_channel||'-')}</span>
                                ${timeDisplay?`<span style="color:var(--border-strong);">·</span><span style="color:${_isScheduled?'var(--success)':'var(--text-secondary)'};font-weight:600;">${window.escapeHtml(timeDisplay)}</span>`:''}
                            </div>
                        </div>
                        <div style="display:flex;gap:6px;flex-shrink:0;align-items:flex-start;" onclick="event.stopPropagation();">
                    ...
                    ${timeBadgeHtml?`<div style="margin-top:4px;">${timeBadgeHtml}</div>`:''}
                </div>`;
            });
        }
        accHtml+=`</div></div>`;
