...
// 💡 1. 툴팁 UI 및 N회차 뱃지용 CSS (모바일 최적화 및 디자인 통일)
...
window.openCustomConfirm = function(title, statusHtml, actionHtml, callbackOrText, btnText = '적용하기') {
    ...
    if(statusHtml) { 
        if($("confirmStateBox")) $("confirmStateBox").style.display = 'block'; 
        if($("confirmSimpleBox")) $("confirmSimpleBox").style.display = 'none'; 
        if($("confirmStatus")) $("confirmStatus").innerHTML = statusHtml; 
        if($("confirmActionState")) $("confirmActionState").innerHTML = actionHtml; 
    } else { 
        if($("confirmStateBox")) $("confirmStateBox").style.display = 'none'; 
        if($("confirmSimpleBox")) $("confirmSimpleBox").style.display = 'block'; 
    ...
    if(btn) {
        ...
        newBtn.onclick = function() {
            if(btnText === '복사하기') { window.copyTxt(callbackOrText, "사전 설문 링크가 복사되었습니다."); window.closeConfirmModal(); } 
                ...
                (async () => {
                    newBtn.disabled = true; newBtn.innerText = "처리 중..."; // 💡 중복 클릭 차단
...
window.saveBlockData = async function() { 
  ...
  if(!payload.block_date || !payload.start_time || !payload.end_time) return showToast("날짜와 시간을 정확히 입력해주세요."); 
...
window.fetchCenterData = async function() {
  try {
    ...
    window.renderCenterData(); window.renderDashboard(); window.renderNoticeData();
...
window.renderCenterData = function() {
  ...
  // [센터 예약 리스트 렌더링 - 취소 버튼 방어 로직]
  try {
      let qRes = ($("searchRes")?.value || "").toLowerCase(); let sRes = $("resSpaceFilter")?.value || "전체";
      let fRes = gRes.filter(r => { 
          let rDate = new Date(r.res_date || r.created_at); let matchSpace = sRes === '전체' || String(r.space_equip||'').includes(sRes);
  ...
  // [수업 신청자 렌더링 - 버튼 UI 통일 & 당일취소 사유 연동]
  try {
      ...
      let fTrnList = gTrn.filter(t => { 
          ...
          if (cInfo.length >= 5) { let tDateObj = new Date(cInfo[0].trim()); tDateObj.setHours(0,0,0,0); if (tDateObj < todayForBlk) return false; } else { let tDate = new Date(t.created_at); if (tDate < oneMonthAgo) return false; }
  ...
  try {
      ...
      if (!isOrdFilter) { monOrders = monOrders.filter(o => !isOrderExpired(o, now)); thuOrders = thuOrders.filter(o => !isOrderExpired(o, now)); }
  ...
  try {
      ...
      if($("blkTableBody")) $("blkTableBody").innerHTML = fBlk.length ? fBlk.map(b=>{ 
          ...
          let capDisplay = '-';
          if (max !== null) {
              if (current >= max) { capDisplay = `<strong style="color:var(--error);">마감 (${max}명)</strong>`; } else { capDisplay = `<strong>${current}</strong> / ${max}`; }
          }
...
window.renderDashboard = async function() {
    ...
    try {
        gRes.forEach(r => {
            ...
            if (calEvts[r.res_date]) {
                let st = String(r.res_time||"").split('~')[0].trim(); let spc = String(r.space_equip||"").split(' ')[0];
    ...
    try {
        ...
        iterDates.forEach(ds => {
            ...
            // 💡 [수정] 캘린더 '더보기' 호버 시 툴팁 상하 잘림 방지 (div 감싸기 & 안전한 패딩 주입)
            if(evts.length > 3) {
                let hiddenText = evts.slice(3).map(e => `<div style="padding:4px 0; line-height:1.4;">${e.time||''} | ${window.escapeHtml(e.text)||''}</div>`).join('');
                evtsHtml += `<div class="dash-cal-more-wrap" style="position:relative;"><div class="dash-cal-more">+${evts.length - 3}건 더보기</div><div class="dash-tooltip" style="text-align:left; white-space:nowrap; font-weight:normal; padding:12px; box-sizing:border-box; z-index:9999; line-height:1.5;">${hiddenText}</div></div>`;
            }
...
window.handlePriceInput = async function(id, val, currentStatus, inputEl) { 
    ...
    let order = gOrd.find(o => String(o.id) === String(id)); if(order) { order.total_price = formatted; order.status = newStatus; } inputEl.value = formatted; 
