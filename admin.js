    ...
    statusConfig.forEach((cfg,gi)=>{
        ...
        if(cfg.items.length===0){
            ...
            cfg.items.forEach(row=>{
                ...
                let _interestHtml=_interestTags.length>0?`<span style="color:var(--text-tertiary);font-size:11px;font-weight:600;">관심</span> <span style="font-size:11px;font-weight:600;color:var(--text-secondary);">${_interestTags.map(t=>window.escapeHtml(t)).join(', ')}</span>`:'';
                let _counselor=(row.counselor_name&&row.counselor_name!=='null')?row.counselor_name:'';
                let _lbl='<span style="color:var(--text-tertiary);font-size:11px;font-weight:600;">';
                let _metaParts=[];
