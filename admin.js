window.renderCenterData = function() {
  ...
  try {
      ...
      // 💡 [수정 1 완벽 반영] '생두 주문 관리' 최상단 타이틀 옆에 툴팁 절대 경로로 안전하게 주입
  ...
  try {
      ...
      fOrd.forEach(o => {
          ...
          // 💡 [수정 2 완벽 반영] 모바일 안쪽에 들어가는 불필요한 뱃지 텍스트 싹 날림 (빈 문자열)
          o._targetBadge = '';
      ...
      // 💡 [수정 3 완벽 반영] 상단 검은색 큰 뱃지에 delivery_date 파싱 값 삽입 (표 위쪽 DOM 타겟팅)
      try {
          if (monTableBody) {
              let monWrap = monTableBody.closest('.table-wrap');
              if (monWrap) {
                  let titleEl = monWrap.previousElementSibling;
                  if (titleEl && monOrders.length > 0) {
                      let tStr = window.formatDeliveryDateFull(monOrders[0].delivery_date); // "4월 27일 월요일"
                      titleEl.innerHTML = `<span style="background:#212529;color:#fff;padding:6px 12px;border-radius:8px;font-size:14px;font-weight:700;display:inline-block;margin-bottom:12px; letter-spacing:-0.5px;">${tStr} 발주</span>`;
                  } else if (titleEl && monOrders.length === 0) {
                      titleEl.innerHTML = `<span style="background:#212529;color:#fff;padding:6px 12px;border-radius:8px;font-size:14px;font-weight:700;display:inline-block;margin-bottom:12px; letter-spacing:-0.5px;">월요일 발주</span>`;
                  }
              }
          }
          ...
          if (thuTableBodyElem) {
              let thuWrap = thuTableBodyElem.closest('.table-wrap');
              if (thuWrap) {
                  let titleEl = thuWrap.previousElementSibling;
                  if (titleEl && thuOrders.length > 0) {
                      let tStr = window.formatDeliveryDateFull(thuOrders[0].delivery_date); // "4월 23일 목요일"
                      titleEl.innerHTML = `<span style="background:#212529;color:#fff;padding:6px 12px;border-radius:8px;font-size:14px;font-weight:700;display:inline-block;margin-bottom:12px; letter-spacing:-0.5px;">${tStr} 발주</span>`;
                  } else if (titleEl && thuOrders.length === 0) {
                      titleEl.innerHTML = `<span style="background:#212529;color:#fff;padding:6px 12px;border-radius:8px;font-size:14px;font-weight:700;display:inline-block;margin-bottom:12px; letter-spacing:-0.5px;">목요일 발주</span>`;
                  }
              }
          }
      } catch(e) {}
...
window.renderCenterData = function() {
  ...
  try {
      ...
      if($("resTableBody")) $("resTableBody").innerHTML = fRes.length ? fRes.map(r=>{ 
          ...
          let actBtn = (String(displayStatus).includes('취소') || displayStatus === '이용완료' || isExpired) 
            ? `<button class="btn-outline btn-sm" disabled style="opacity
