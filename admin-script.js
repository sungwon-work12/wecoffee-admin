...
window.renderCenterData = function() {
  ...
  $("ordTableBody").innerHTML = fOrd.length ? fOrd.map(o=>{ 
    ...
    if(m) {
        cNm = m[1].trim(); 
        let dStr = m[4]+"요일";
        dOptHtml = `<div style="margin-top:6px; font-size:12px; font-weight:700; color:var(--primary); background:var(--primary-light); padding:4px 8px; border-radius:4px; display:inline-block;">🎯 ${m[2]}/${m[3]}(${m[4]}) 마감발주</div>`;
    } else {
        let oM = rNm.match(/(.+) \[(.*?)\]/);
        if(oM) {
            cNm = oM[1].trim();
            dOptHtml = `<div style="margin-top:6px; font-size:12px; font-weight:700; color:var(--primary); background:var(--primary-light); padding:4px 8px; border-radius:4px; display:inline-block;">🎯 희망: ${oM[2].trim()}</div>`;
        }
    }
    ...
    return `<tr>${mPreview}
        ...
        <td data-label="성함(기수)"><strong style="font-weight:800;">${o.name}</strong></td>
...
window.downloadExcel = function(type) {
  ...
  if(type === 'applications') { data = globalApps; filename = "가입신청"; headers = ['신청일', '기수', '성함', '연락처', '관심분야', '유입경로', '진행상황', '가입여부', '상담일시', '담당자']; } else if(type === 'members') { data = globalMembers; filename = "멤버리스트"; headers = ['등록일', '상태', '기수', '성함', '연락처', '활동종료일']; } else if(type === 'reservations') { data = gRes; filename = "예약현황"; headers = ['접수일', '기수', '성함', '연락처', '예약날짜', '예약시간', '센터', '장비', '상태', '취소사유']; } else if(type === 'trainings') { data = gTrn; filename = "수업훈련"; headers = ['신청일', '기수', '성함', '연락처', '콘텐츠', '상태', '취소사유']; } else if(type === 'orders') { data = gOrd; filename = "생두주문"; headers = ['주문일', '주문번호', '기수', '성함', '연락처', '수령센터', '생두사', '상품명', '희망수령일', '수량', '총금액', '상태']; }
